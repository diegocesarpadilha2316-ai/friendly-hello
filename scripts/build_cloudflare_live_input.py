...
while stack:
    ...
    for spec in pattern.findall(path.read_text()):
        child = (path.parent / spec[2:]).relative_to(ROOT).as_posix()
        if child not in seen:
            stack.append(child)
modules = [ROOT / rel for rel in sorted(seen)]
env_values = {}
...
Path('/home/ubuntu/cloudflare-worker-live-input.json').write_text(json.dumps({'code': code}))
print(f'modules={len(modules)} input_bytes={Path("/tmp/cloudflare-worker-input.json").stat().st_size}')
