    ...
    <PageContainer>
      ...
      {tab === "namespaces" && (
        <NamespacesTab
          ...
          onInvalidate={(ns) => invalidate.mutate({ scope: "namespace", target: ns })}
        />
      )}
      {tab === "tags" && (
        <TagsTab
          data={tagCounts}
          onInvalidate={(tag) => invalidate.mutate({ scope: "tag", target: tag })}
        />
      )}
  ...
  if (!data.length)
    return (
      <EmptyState
        ...
        description="Módulos podem operar sem namespaces declarados. Registre-os para configurar TTL padrão e estratégia."
      />
    );
  return <DataTable data={[...data]} columns={cols} getRowKey={(r) => r.id} />;
}
...
function InvalidationsTab({ data }: { data: readonly CacheInvalidation[] }) {
  const cols: DataTableColumn<CacheInvalidation>[] = [
    ...
    },
    {
      id: "target",
      header: "Alvo",
      cell: (r) => <code className="text-xs break-all">{r.target}</code>,
    },
