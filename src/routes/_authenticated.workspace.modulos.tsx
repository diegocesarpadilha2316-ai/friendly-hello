  ...
  head: () => ({
    meta: [
      { title: "Módulos Contratados — Workspace | Dioris Hub" },
      {
        name: "description",
        content:
          "Central de módulos: contratação, consumo, favoritos, saúde e histórico com reuso total do Core.",
      },
...
// -----------------------------------------------------------------------------
type CategoryKey =
  "planejamento" | "criacao" | "comercial" | "financeiro" | "automacao" | "ia" | "plataforma";
...
function ModulosPage() {
  ...
  const [tab, setTab] = useState<
    "grade" | "categorias" | "favoritos" | "historico" | "eventos" | "saude"
  >("grade");
  ...
  const enriched: EnrichedModule[] = useMemo(
    () =>
      modules.map((m) => {
        const contracted =
          planFeatures.has(m.id) ||
          planFeatures.has(m.slug) ||
          planFeatures.has(m.label.toLowerCase());
  ...
  const filtered = enriched.filter((m) => {
    if (query && !`${m.label} ${m.description}`.toLowerCase().includes(query.toLowerCase()))
      return false;
    ...
    <PageContainer>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-5">
        ...
        <MetricCard
          ...
          label="Ativos"
          value={kpis.active}
          hint={billing.summary.plan?.label ?? "Sem plano"}
        />
        <MetricCard
        ...
        <MetricCard
          ...
          value={kpis.consumo.toLocaleString("pt-BR")}
        />
      </div>
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {new Date(m.lastAccess!).toLocaleDateString("pt-BR")}
                  </span>
                </button>
              );
            })}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <div className="min-w-[240px] flex-1">
          <SearchInput
            ...
            placeholder="Pesquisa global (nome, descrição)…"
          />
        </div>
        <Select
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="mt-6">
        ...
        <TabsContent value="grade" className="mt-6">
          {billing.isLoading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filtered.map((m) => (
                <ModuleGridCard key={m.id} m={m} onOpen={openModule} onFavorite={toggleFavorite} />
              ))}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="favoritos" className="mt-6">
          {enriched.filter((m) => m.favorite).length === 0 ? (
            <EmptyState
              ...
              title="Sem favoritos"
              description="Clique no ícone de estrela nos cards para favoritar módulos."
            />
          ) : (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {enriched
        ...
        <TabsContent value="historico" className="mt-6">
          <HistoryTab enriched={enriched} />
        </TabsContent>
        <TabsContent value="eventos" className="mt-6">
          <EventsPanel />
        </TabsContent>
        <TabsContent value="saude" className="mt-6">
          <HealthPanel enriched={enriched} />
        </TabsContent>
      </Tabs>
      ...
      <FormSection
        ...
        description="Recursos incluídos em cada plano."
      >
        {plans.isLoading ? (
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {plans.plans.map((p) => (
              <div
                key={p.key}
                className={`rounded-lg border p-4 ${billing.summary.plan?.key === p.key ? "border-primary bg-primary/5" : "border-border"}`}
              >
                <div className="flex items-baseline justify-between">
                  <h3 className="text-sm font-semibold">{p.label}</h3>
                  <span className="text-xs text-muted-foreground">
                    {p.monthlyCredits.toLocaleString("pt-BR")} créd/mês
                  </span>
                </div>
      ...
      <ModuleCard
        ...
        description={
          <span className="block">
            <span className="text-xs text-muted-foreground">
              {CATEGORY_LABEL[m.category]} · v{m.version} · Plano {m.requiredPlan}
            </span>
            ...
            <span className="mt-2 flex gap-3 text-[11px] text-muted-foreground">
              ...
              {m.lastAccess ? (
                <span>· Último: {new Date(m.lastAccess).toLocaleDateString("pt-BR")}</span>
              ) : null}
            </span>
          </span>
        }
          ...
          <>
            <SheetHeader>
              <SheetTitle className="flex items-center gap-2">
                ...
                {m.label}
                <StatusBadge tone={STATUS_TONE[m.extStatus]}>
                  {STATUS_LABEL[m.extStatus]}
                </StatusBadge>
              </SheetTitle>
            ...
            <div className="mt-6 space-y-6 text-sm">
              ...
              <section>
                ...
                <p className="text-xs text-muted-foreground">
                  Core (Auth, Tenant, Billing, EventBus, NotificationManager, Observability, IA
                  Gateway).
                </p>
              </section>
              <section>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  size="sm"
                  disabled={m.extStatus !== "ativo" && m.extStatus !== "trial"}
                  asChild={m.extStatus === "ativo" || m.extStatus === "trial"}
                >
                  {m.extStatus === "ativo" || m.extStatus === "trial" ? (
                    <a href={m.path}>
                      Abrir módulo <ExternalLink className="ml-1 h-3.5 w-3.5" />
                    </a>
                  ) : (
            ...
            {enriched
              ...
              .map((m) => (
                <li key={m.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {new Date(m.lastAccess!).toLocaleString("pt-BR")}
                  </span>
                </li>
              ))}
          </ul>
  ...
  if (rows.length === 0)
    return (
      <EmptyState
        ...
        title="Sem eventos"
        description="Eventos do EventBus aparecerão aqui conforme os módulos publicarem."
      />
    );
  return (
