...
export const FurnitureRenderer: React.FC<{ singleItemId?: string; demoItem?: any }> = ({
  singleItemId,
  demoItem,
}) => {
    ...
    <group name="furniture-layer">
      {items.map((item) => {
        ...
        if (item.family === "kitchen-base-cabinet") {
          return <BaseCabinetMesh key={item.id} item={item} onSelect={selectItem} />;
        }

        // Bridge to existing parametric families for other furniture types
        return <LegacyAssemblyRenderer key={item.id} item={item} onSelect={selectItem} />;
      })}
    </group>
  );
};
