...
export function FabricationPanel() {
  ...
  const aiHit = useMemo<FabricationIntent | null>(
    () => findIntentByQuery(intents, aiQuery),
    [intents, aiQuery],
  );
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-lg border border-primary/30 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 p-4">
        <div className="text-xs uppercase tracking-widest text-primary">
          Fase 3.13 · Motor de Fabricação
        </div>
        <div className="text-lg font-semibold">Plano de Corte Inteligente + CNC</div>
        <div className="mt-1 text-xs text-muted-foreground">
          Algoritmo próprio (Guillotine + Best-Fit + Rotação + Reuso de Sobra) · Kerf{" "}
          {constraints.kerfMm}mm · Margem {constraints.marginMm}mm · Rotação{" "}
          {constraints.allowRotation ? "on" : "off"}
        </div>
      </div>
      ...
      {sub === "otimizador" && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <MetricCard
            ...
            value={`${Math.round(plan.totals.avgUsageRatio * 100)}%`}
            hint="médio por chapa"
          />
          <MetricCard
            label="Efetivo c/ sobra"
            value={`${Math.round(plan.totals.effectiveRatio * 100)}%`}
            hint="reuso ativo"
          />
          <MetricCard
            label="Cortes"
            value={plan.totals.totalCuts}
            hint={`grão respeitado ${plan.totals.grainRespectedPct}%`}
          />
          <MetricCard
            label="Desperdício"
            value={`${plan.totals.wasteAreaM2.toFixed(2)} m²`}
            hint="perda líquida"
          />
          <MetricCard
            label="Sobra útil"
            value={`${plan.totals.offcutAreaM2.toFixed(2)} m²`}
            hint="≥ 0,15 m²"
          />
          <MetricCard label="Não colocadas" value={plan.unplaced.length} hint="revise dimensões" />
          <MetricCard
            label="Peças"
            value={report.totals.parts}
            hint={`${report.totals.modules} módulos`}
          />
        </div>
      )}
            ...
            columns={[
              {
                id: "code",
                header: "Código",
                cell: (s) => <span className="font-mono text-xs">{s.partCode}</span>,
              },
              { id: "name", header: "Peça", cell: (s) => s.partName },
              {
                id: "holes",
                header: "Furos",
                cell: (s) => <span className="tabular-nums">{s.totalHoles}</span>,
                align: "right",
              },
              {
                id: "eta",
                header: "Tempo (s)",
                cell: (s) => <span className="tabular-nums">{s.estimatedSeconds}</span>,
                align: "right",
              },
            ]}
            data={[...drilling]}
      ...
      {sub === "cnc" && (
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap gap-2">
            ...
            <select
              value={format}
              onChange={(e) => setFormat(e.target.value as CamFormat)}
              className="rounded-md border border-border bg-background px-3 py-1.5 text-xs"
            >
                ...
                <div key={row.code} className="rounded-lg border border-border bg-card p-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <StatusBadge tone="info">
                      {pp.format.toUpperCase()} · ~{pp.estimatedMinutes}min
                    </StatusBadge>
                  </div>
          ...
          <details className="rounded-lg border border-border bg-card p-3">
            <summary className="cursor-pointer text-xs text-muted-foreground">
              Ver bundle completo ({report.cutList.length} peças)
            </summary>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {FABRICATION_EXPORTS.map((ex) => (
            ...
            >
              ...
              <StatusBadge
                tone={
                  ex.target === "cnc" ? "warning" : ex.target === "producao" ? "success" : "info"
                }
              >
                {ex.target}
              </StatusBadge>
...
function PlanBoards({ boards }: { boards: readonly FabricationBoard[] }) {
  const cols: DataTableColumn<FabricationBoard>[] = [
    ...
    },
    {
      id: "used",
      header: "Usado (m²)",
      cell: (b) => <span className="tabular-nums">{b.usedM2.toFixed(2)}</span>,
      align: "right",
    },
    {
      id: "waste",
      header: "Desperdício (m²)",
      cell: (b) => <span className="tabular-nums">{b.wasteM2.toFixed(2)}</span>,
      align: "right",
    },
    {
      id: "offcut",
      header: "Sobra útil",
      cell: (b) => <span className="tabular-nums">{b.offcuts.length}</span>,
      align: "right",
    },
    {
      id: "ratio",
      header: "Aproveit.",
      cell: (b) => (
        <StatusBadge tone={b.usageRatio >= 0.75 ? "success" : "warning"}>
    ...
    },
    {
      id: "cuts",
      header: "Cortes",
      cell: (b) => <span className="tabular-nums">{b.cutOrder.length}</span>,
      align: "right",
    },
  ];
  return (
        ...
        {boards.map((board) => (
          <div key={board.index} className="rounded-lg border border-border bg-card p-3">
            ...
            <svg
              ...
              className="h-44 w-full rounded bg-muted/40"
            >
              <rect
                x={0}
                y={0}
              ...
              {board.placements.map((pl, i) => (
                <g key={i}>
                  ...
                  {pl.rotated && (
                    <text x={pl.x + 14} y={pl.y + 90} fontSize={28} fill="hsl(var(--accent))">
                      ↻
                    </text>
                  )}
                </g>
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Ordem de corte: {board.cutOrder.join(" → ")} · sobra útil: {board.offcuts.length}{" "}
              peça(s)
            </div>
          </div>
        ))}
