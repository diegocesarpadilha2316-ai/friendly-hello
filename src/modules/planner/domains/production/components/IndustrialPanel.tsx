  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-lg border border-accent/30 bg-gradient-to-br from-accent/5 via-transparent to-primary/5 p-4">
        <div className="text-xs uppercase tracking-widest text-accent">Fase 3.14 · Produção Industrial</div>
        <div className="text-lg font-semibold">Nesting + CNC + Fábrica 4.0</div>
        <div className="mt-1 text-xs text-muted-foreground">
          Camada aditiva 100% derivada — sobras, montagem, custo, IA industrial e etiquetas QR/Barcode.
        </div>
      </div>
      ...
      {sub === "montagem" && <AssemblySub steps={industrial.assembly.steps} totalMinutes={industrial.assembly.totalMinutes} />}
      {sub === "reotimizar" && <ReoptimizeSub compare={industrial.compare} onRun={industrial.reoptimize} />}
      {sub === "custo" && industrial.cost && (
        <CostSub rows={industrial.cost.rows} total={industrial.cost.final} marginPct={industrial.cost.marginPct} costPerM2={industrial.cost.costPerM2} pricePerM2={industrial.cost.pricePerM2} />
      )}
...
function BoardsSub({ plan }: { plan: ReturnType<typeof useFabrication>["plan"] }) {
  ...
  if (!plan) {
    return <EmptyState title="Sem plano de fabricação" description="Aguardando otimização das chapas." />;
  }
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        <Button variant="outline" onClick={() => setZoom((z) => Math.max(0.5, z - 0.25))}>−</Button>
        <span className="tabular-nums text-xs text-muted-foreground">{Math.round(zoom * 100)}%</span>
        <Button variant="outline" onClick={() => setZoom((z) => Math.min(3, z + 0.25))}>+</Button>
        {selected && <StatusBadge tone="info">Peça selecionada: {selected}</StatusBadge>}
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {plan.boards.map((board) => (
          <div key={board.index} className="overflow-hidden rounded-lg border border-border bg-card p-3">
            <div className="mb-2 flex items-center justify-between text-sm">
              <span className="font-medium">Chapa #{board.index} · {board.spec.material}</span>
            ...
            <div className="overflow-auto">
              ...
              >
                ...
                {board.offcuts.map((rect, i) => (
                  <rect key={`o${i}`} x={rect.x} y={rect.y} width={rect.w} height={rect.h} fill="hsl(var(--accent) / 0.18)" stroke="hsl(var(--accent))" strokeWidth={1} strokeDasharray="8 6" />
                ))}
                {board.placements.map((pl, i) => (
                  <g key={i} onClick={() => setSelected(pl.code)} style={{ cursor: "pointer" }}>
                    <rect
                      ...
                      width={pl.w}
                      height={pl.h}
                      fill={selected === pl.code ? "hsl(var(--primary) / 0.55)" : "hsl(var(--primary) / 0.3)"}
                      stroke="hsl(var(--primary))"
                      strokeWidth={selected === pl.code ? 6 : 2}
                    />
                    <text x={pl.x + 16} y={pl.y + 46} fontSize={36} fill="hsl(var(--primary-foreground))">
                      {pl.code} {pl.rotated ? "↻" : ""}
                    </text>
                    <text x={pl.x + 16} y={pl.y + 92} fontSize={26} fill="hsl(var(--primary-foreground))">
...
function OffcutsSub({
  ...
  const columns: DataTableColumn<OffcutInventoryItem>[] = [
    { id: "dim", header: "Tamanho", cell: (o) => <span className="tabular-nums">{o.lengthMm}×{o.widthMm} mm</span> },
    { id: "esp", header: "Esp.", cell: (o) => <span className="tabular-nums">{o.thicknessMm} mm</span>, align: "right" },
    { id: "mat", header: "Material", cell: (o) => <div><div className="text-xs font-medium">{o.material}</div><div className="text-[11px] text-muted-foreground">{o.brand} · {o.color}</div></div> },
    { id: "area", header: "Área", cell: (o) => <span className="tabular-nums">{o.areaM2.toFixed(3)} m²</span>, align: "right" },
    { id: "origem", header: "Origem", cell: (o) => <div className="text-xs"><div>{o.projectName}</div><div className="text-muted-foreground">{new Date(o.createdAt).toLocaleDateString("pt-BR")}</div></div> },
    { id: "status", header: "Status", cell: (o) => (
      <select
        value={o.status}
    ...
    { id: "acoes", header: "", cell: (o) => <Button variant="ghost" onClick={() => onDelete(o.id)}>Excluir</Button>, align: "right" },
  ];
  const totalArea = offcuts.reduce((a, o) => a + o.areaM2, 0);
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        <Button onClick={onRegister} disabled={!hasPlan}>Registrar sobras do plano atual</Button>
        <Button variant="outline" onClick={onClear} disabled={offcuts.length === 0}>Limpar inventário</Button>
        <StatusBadge tone="info">{offcuts.length} sobras · {totalArea.toFixed(2)} m²</StatusBadge>
      </div>
      {offcuts.length === 0 ? (
        <EmptyState title="Nenhuma sobra cadastrada" description="Rode o Otimizador (Fabricação) e clique em 'Registrar sobras do plano atual' para popular o inventário." />
      ) : (
...
function AssemblySub({ steps, totalMinutes }: { steps: readonly AssemblyStep[]; totalMinutes: number }) {
  if (steps.length === 0) {
    return <EmptyState title="Sem passos" description="Insira móveis para gerar a sequência de montagem." />;
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <StatusBadge tone="success">≈ {Math.round(totalMinutes / 60)}h {totalMinutes % 60}min</StatusBadge>
      </div>
        ...
        {[...groups.entries()].map(([label, list]) => (
          <div key={label} className="rounded-lg border border-border bg-card p-3">
            ...
            <ol className="flex flex-col gap-2">
              {list.map((s) => (
                <li key={s.order} className="rounded-md border border-border/60 bg-muted/30 p-2 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="font-medium">#{s.order} · {s.title}</span>
                    <span className="tabular-nums text-muted-foreground">{s.estimatedMinutes} min</span>
                  </div>
                  <div className="mt-1 text-[11px] text-muted-foreground">{s.description}</div>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {s.toolset.map((t) => (
                      <span key={t} className="rounded bg-background px-1 py-0.5 text-[10px] text-muted-foreground">{t}</span>
                    ))}
                  </div>
                </li>
              ))}
    ...
    <div className="flex flex-col gap-3">
      <div className="grid gap-3 rounded-lg border border-border bg-card p-3 md:grid-cols-4">
        <label className="flex flex-col gap-1 text-xs">
          ...
          <input type="number" value={margin} onChange={(e) => setMargin(Number(e.target.value) || 0)} className="rounded border border-border bg-background px-2 py-1" />
        </label>
        <label className="flex flex-col gap-1 text-xs">
          <span className="text-muted-foreground">Kerf (mm)</span>
          <input type="number" value={kerf} onChange={(e) => setKerf(Number(e.target.value) || 0)} className="rounded border border-border bg-background px-2 py-1" />
        </label>
      ...
      {compare ? (
        <div className="grid gap-3 md:grid-cols-3">
          <MetricCard label="Chapas" value={`${compare.before.boards} → ${compare.after.boards}`} hint={`Δ ${compare.diff.boards >= 0 ? "-" : "+"}${Math.abs(compare.diff.boards)}`} />
          <MetricCard label="Aproveitamento" value={`${compare.before.usagePct}% → ${compare.after.usagePct}%`} hint={`Δ ${compare.diff.usagePct >= 0 ? "+" : ""}${compare.diff.usagePct}%`} />
          <MetricCard label="Economia estimada" value={fmtBRL(compare.economyBRL)} hint={`sobra: ${compare.before.wasteM2.toFixed(2)} m² → ${compare.after.wasteM2.toFixed(2)} m²`} />
        </div>
      ) : (
        <div className="rounded-lg border border-dashed border-border bg-card p-6 text-center text-sm text-muted-foreground">
          Ajuste os parâmetros e clique em <span className="font-medium">Otimizar novamente</span> para comparar antes × depois.
        </div>
      )}
    </div>
  );
...
function CostSub({
  ...
  const columns: DataTableColumn<IndustrialCostRow>[] = [
    { id: "group", header: "Grupo", cell: (r) => <StatusBadge tone="neutral">{r.group}</StatusBadge> },
    { id: "label", header: "Descrição", cell: (r) => <div><div className="font-medium">{r.label}</div><div className="text-xs text-muted-foreground">{r.hint}</div></div> },
    { id: "val", header: "Valor", cell: (r) => <span className="tabular-nums font-medium">{fmtBRL(r.value)}</span>, align: "right" },
  ];
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-4 text-sm">
        <div className="flex justify-between"><span className="text-muted-foreground">R$/m² custo</span><span className="tabular-nums">{fmtBRL(costPerM2)}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">R$/m² venda</span><span className="tabular-nums">{fmtBRL(pricePerM2)}</span></div>
        <div className="flex justify-between"><span className="text-muted-foreground">Margem real</span><span className="tabular-nums">{marginPct.toFixed(1)}%</span></div>
        <div className="my-2 h-px bg-border" />
        <div className="flex justify-between text-base"><span className="font-semibold">Preço final</span><span className="tabular-nums font-semibold">{fmtBRL(total)}</span></div>
      </div>
    </div>
  );
}
...
function LabelsSub({ labels }: { labels: readonly PartLabel[] }) {
  if (labels.length === 0) {
    return <EmptyState title="Sem etiquetas" description="Insira móveis para gerar etiquetas com QR e código de barras." />;
  }
  return (
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3">
      ...
      <div className="flex gap-3">
        <svg viewBox={`0 0 ${matrix.length} ${matrix.length}`} className="h-24 w-24 rounded bg-background p-1">
          {matrix.map((row, y) =>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        {intents.map((i) => (
          <button
            key={i.id}
            onClick={() => { setQ(i.question); setHit(i); }}
