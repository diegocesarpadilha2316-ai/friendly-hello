...
export function ProductionStudio() {
  ...
  const partsFiltered = useMemo(() => {
    ...
    return report.parts.filter((p) =>
      [p.label, p.material, p.finish, p.furnitureLabel, p.roomLabel, p.category]
  ...
  const cutListFiltered = useMemo(() => {
    ...
    return report.cutList.filter((r) => [r.code, r.name, r.material, r.brand].some((f) => f.toLowerCase().includes(q)));
  }, [report, query]);
  ...
  const runAi = () => {
    ...
    if (cmd) {
      ...
      setAiTrace({ label: "Comando não reconhecido", hint: "Tente: gerar lista de corte, plano, orçamento, etiquetas, CNC." });
    }
  };
  ...
  const downloadFile = (format: ProductionExportFormat) => {
    ...
    if (format === "csv") {
      ...
      content = `<?xml version="1.0"?>\n<dioris-production project="${projectName}">\n${report.cutList
        .map((r) => `  <part code="${r.code}" material="${r.material}" length="${r.lengthMm}" width="${r.widthMm}" qty="${r.qty}"/>`)
        .join("\n")}\n</dioris-production>\n`;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card px-4 py-3">
        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Production Studio · Dioris</div>
          <div className="text-lg font-semibold">{projectName}</div>
          <div className="text-xs text-muted-foreground">Cliente: {clientName} · {report.totals.modules} módulos · {report.totals.parts} peças</div>
        </div>
      ...
      {tab === "plano" && <CuttingPlanPanel boards={report.cuttingPlan.boards} totals={report.cuttingPlan.totals} />}
      ...
      {tab === "etiquetas" && <LabelsPanel labels={report.labels.filter((l) => !query || l.code.toLowerCase().includes(query.toLowerCase()) || l.moduleLabel.toLowerCase().includes(query.toLowerCase()))} />}
      {tab === "cnc" && (
        <CncPanel
...
function DashboardPanel({ report }: { report: NonNullable<ReturnType<typeof useProduction>["report"]> }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
      ...
      <MetricCard label="Chapas" value={report.cuttingPlan.totals.boardsCount} hint={`≈ ${fmtNum(report.totals.boardsM2)} m²`} />
      <MetricCard label="Aproveitamento" value={`${usagePct}%`} hint={`sobra ${fmtNum(report.cuttingPlan.totals.wasteAreaM2)} m²`} trend={{ value: usagePct >= 75 ? "ok" : "revisar", direction: usagePct >= 75 ? "up" : "down" }} />
      <MetricCard label="Fita de borda" value={`${fmtNum(report.totals.edgeMeters)} m`} hint="total linear" />
      <MetricCard label="Peso estimado" value={`${fmtNum(report.totals.weightKg)} kg`} hint="chapas + partes" />
      <MetricCard label="Tempo total" value={`${report.time.totalH} h`} hint={`corte ${report.time.cuttingH}h · mont. ${report.time.assemblyH}h`} />
      <MetricCard label="Preço final" value={fmtBRL(report.budget.summary.final)} hint={`${fmtBRL(report.budget.summary.perM2)}/m² · margem ${report.budget.parameters.marginPct}%`} trend={{ value: "orçado", direction: "up" }} />
    </div>
  );
}
...
function PartsPanel({ parts }: { parts: readonly ProductionPart[] }) {
  const columns: DataTableColumn<ProductionPart>[] = [
    { id: "cat", header: "Categoria", cell: (p) => <StatusBadge tone="neutral">{p.category}</StatusBadge> },
    { id: "label", header: "Peça", cell: (p) => <div><div className="font-medium">{p.label}</div><div className="text-xs text-muted-foreground">{p.furnitureLabel} · {p.roomLabel}</div></div> },
    { id: "dim", header: "Dimensão", cell: (p) => <span className="tabular-nums">{p.widthMm} × {p.heightMm} × {p.thicknessMm}</span>, align: "right" },
    { id: "qty", header: "Qtd", cell: (p) => <span className="tabular-nums">{p.qty}</span>, align: "right" },
    { id: "mat", header: "Material", cell: (p) => <span className="text-xs">{p.material}</span> },
    { id: "peso", header: "Peso (kg)", cell: (p) => <span className="tabular-nums">{fmtNum(p.weightKg * p.qty)}</span>, align: "right" },
  ];
...
function CutListPanel({ rows }: { rows: readonly CutListRow[] }) {
  const columns: DataTableColumn<CutListRow>[] = [
    { id: "code", header: "Código", cell: (r) => <span className="font-mono text-xs">{r.code}</span> },
    { id: "name", header: "Nome", cell: (r) => r.name },
    { id: "mat", header: "Material", cell: (r) => <span className="text-xs">{r.material}</span> },
    { id: "esp", header: "Esp.", cell: (r) => <span className="tabular-nums">{r.thicknessMm} mm</span>, align: "right" },
    { id: "dim", header: "Compr × Larg", cell: (r) => <span className="tabular-nums">{r.lengthMm} × {r.widthMm}</span>, align: "right" },
    { id: "qty", header: "Qtd", cell: (r) => <span className="tabular-nums">{r.qty}</span>, align: "right" },
    { id: "veio", header: "Veio", cell: (r) => r.grain },
    { id: "fita", header: "Fita", cell: (r) => <span className="text-xs">{r.edgeTape}</span> },
    { id: "area", header: "Área (m²)", cell: (r) => <span className="tabular-nums">{fmtNum(r.areaM2, 3)}</span>, align: "right" },
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <MetricCard label="Área usada" value={`${fmtNum(totals.usedAreaM2)} m²`} hint="soma das peças" />
        <MetricCard label="Sobra" value={`${fmtNum(totals.wasteAreaM2)} m²`} hint="desperdício previsto" />
        <MetricCard label="Aproveitamento médio" value={`${Math.round(totals.avgUsageRatio * 100)}%`} />
      </div>
        ...
        {boards.map((board) => (
          <div key={board.index} className="rounded-lg border border-border bg-card p-3">
            <div className="mb-2 flex items-center justify-between text-sm">
              ...
              <StatusBadge tone={board.usageRatio >= 0.75 ? "success" : "warning"}>{Math.round(board.usageRatio * 100)}%</StatusBadge>
            </div>
            <svg viewBox={`0 0 ${board.spec.lengthMm} ${board.spec.widthMm}`} className="h-40 w-full rounded bg-muted/50">
              ...
              {board.placements.map((pl, i) => (
                <g key={i}>
                  ...
                  <text x={pl.x + 12} y={pl.y + 40} fontSize={36} fill="hsl(var(--primary-foreground))">{pl.code}</text>
                </g>
              ))}
            </svg>
            <div className="mt-2 text-xs text-muted-foreground">
              {board.spec.brand} · {board.spec.material} · usado {fmtNum(board.usedM2)} m² · sobra {fmtNum(board.wasteM2)} m²
            </div>
          </div>
        ))}
      </div>
...
function HardwarePanel({ rows }: { rows: readonly HardwareBomRow[] }) {
  ...
  const columns: DataTableColumn<HardwareBomRow>[] = [
    ...
    { id: "label", header: "Descrição", cell: (r) => <div><div className="font-medium">{r.label}</div><div className="text-xs text-muted-foreground">{r.code}</div></div> },
    { id: "qty", header: "Qtd", cell: (r) => <span className="tabular-nums">{r.qty} {r.unit}</span>, align: "right" },
    { id: "unit", header: "Unit.", cell: (r) => <span className="tabular-nums">{fmtBRL(r.unitPrice)}</span>, align: "right" },
    { id: "total", header: "Total", cell: (r) => <span className="tabular-nums font-medium">{fmtBRL(r.total)}</span>, align: "right" },
  ];
  return (
    <div className="flex flex-col gap-3">
      <DataTable columns={columns} data={[...rows]} getRowKey={(r) => `${r.kind}-${r.code}`} />
      <div className="flex justify-end text-sm"><span className="text-muted-foreground">Total ferragens:&nbsp;</span><span className="font-semibold">{fmtBRL(total)}</span></div>
    </div>
  );
}

function BudgetPanel({ budget }: { budget: NonNullable<ReturnType<typeof useProduction>["report"]>["budget"] }) {
  const columns: DataTableColumn<BudgetLine>[] = [
    { id: "group", header: "Grupo", cell: (l) => <StatusBadge tone="neutral">{l.group}</StatusBadge> },
    { id: "label", header: "Descrição", cell: (l) => l.label },
    { id: "qty", header: "Qtd", cell: (l) => <span className="tabular-nums">{l.qty} {l.unit}</span>, align: "right" },
    { id: "unit", header: "Unit.", cell: (l) => <span className="tabular-nums">{fmtBRL(l.unitPrice)}</span>, align: "right" },
    { id: "total", header: "Total", cell: (l) => <span className="tabular-nums font-medium">{fmtBRL(l.total)}</span>, align: "right" },
  ];
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-4 text-sm">
        ...
        <SummaryRow label={`Overhead ${budget.parameters.overheadPct}%`} value={fmtBRL(s.overhead)} />
...
function TimePanel({ time }: { time: NonNullable<ReturnType<typeof useProduction>["report"]>["time"] }) {
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
      ...
      <MetricCard label="Total" value={`${time.totalH} h`} hint="linha de produção" trend={{ value: "estimado", direction: "up" }} />
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3">
      ...
      <DataTable
        columns={[
          ...
          { id: "stage", header: "Etapa", cell: (o) => <StatusBadge tone="info">{o.stage}</StatusBadge> },
          { id: "parts", header: "Peças", cell: (o) => <span className="tabular-nums">{o.parts}</span>, align: "right" },
          { id: "prog", header: "Progresso", cell: (o) => (
            <div className="w-32">
              <div className="h-2 rounded-full bg-muted"><div className="h-2 rounded-full bg-primary" style={{ width: `${o.progress}%` }} /></div>
      ...
      {labels.slice(0, 60).map((l) => (
        <div key={l.code} className="rounded-lg border border-border bg-card p-3">
          ...
          <div className="mt-2 flex items-center gap-3">
            <div className="grid grid-cols-8 gap-[1px] rounded border border-border p-1" aria-hidden>
              {Array.from({ length: 64 }).map((_, i) => (
                <span key={i} className={cn("h-2 w-2", (l.code.charCodeAt(i % l.code.length) + i) % 3 === 0 ? "bg-foreground" : "bg-transparent")} />
              ))}
            </div>
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold">{l.moduleLabel} · {l.partLabel}</div>
              <div className="truncate text-xs text-muted-foreground">{l.projectName} · {l.roomLabel}</div>
              <div className="mt-1 truncate text-xs">{l.dimensions}</div>
              <div className="mt-1 flex gap-2">
                <span className="font-mono text-[11px]">{l.code}</span>
                <span className="font-mono text-[11px] text-muted-foreground">|||{l.barcodePayload}|||</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3">
        ...
        {CNC_MACHINES.map((m) => (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">{m.kind} · {m.formats.join(" / ")}</span>
            <StatusBadge tone={m.status === "beta" ? "warning" : "neutral"} className="mt-1">{m.status}</StatusBadge>
          </button>
        ))}
      </div>
      <div className="flex flex-col gap-3">
        <DataTable
          columns={[
            { id: "code", header: "Peça", cell: (j) => <span className="font-mono text-xs">{j.code}</span> },
            { id: "fmt", header: "Formato", cell: (j) => <StatusBadge tone="info">{j.format}</StatusBadge> },
            { id: "ops", header: "Operações", cell: (j) => <span className="text-xs text-muted-foreground">{j.operations.join(" › ")}</span> },
            { id: "t", header: "Tempo (min)", cell: (j) => <span className="tabular-nums">{j.estimatedMinutes}</span>, align: "right" },
        ...
        {first && (
          <div className="grid gap-3 md:grid-cols-2">
            <pre className="max-h-64 overflow-auto rounded-lg border border-border bg-muted/40 p-3 text-[11px]"><code>{toGcodeStub(first)}</code></pre>
            <pre className="max-h-64 overflow-auto rounded-lg border border-border bg-muted/40 p-3 text-[11px]"><code>{toDxfStub(first)}</code></pre>
          </div>
        )}
      </div>
    </div>
...
function ErpPanel() {
  const columns: DataTableColumn<ErpProvider>[] = [
    { id: "label", header: "ERP", cell: (p) => <div><div className="font-medium">{p.label}</div><div className="text-xs text-muted-foreground">{p.category}</div></div> },
    { id: "scopes", header: "Escopos", cell: (p) => <div className="flex flex-wrap gap-1">{p.scopes.map((s) => <StatusBadge key={s} tone="neutral">{s}</StatusBadge>)}</div> },
    { id: "status", header: "Status", cell: (p) => <StatusBadge tone={p.status === "beta" ? "warning" : "info"}>{p.status}</StatusBadge> },
    { id: "action", header: "Ação", cell: () => <Button size="sm" variant="outline" disabled>Preparar integração</Button>, align: "right" },
  ];
  return (
    <div className="flex flex-col gap-3">
      <div className="rounded-lg border border-dashed border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        Interfaces preparadas para ERP nacional (Tiny, Bling, Omie, Conta Azul) e enterprise (SAP, TOTVS). Integração real será liberada em fase futura.
      </div>
      <DataTable columns={columns} data={[...(ERP_PROVIDERS as CncTargetMachine[] & ErpProvider[])]} getRowKey={(p) => (p as ErpProvider).id} />
    </div>
  );
}
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {PRODUCTION_EXPORTS.map((exp) => (
        <div key={exp.format} className="flex flex-col gap-2 rounded-lg border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold">{exp.label}</div>
            <StatusBadge tone="info">.{exp.extension}</StatusBadge>
          </div>
          <p className="text-xs text-muted-foreground">{exp.description}</p>
          <Button size="sm" onClick={() => onDownload(exp.format)}>Baixar</Button>
        </div>
      ))}
    </div>
  );
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <p className="text-xs text-muted-foreground">
          Descreva o que deseja em linguagem natural — a IA identifica o comando e abre o painel correspondente. Nenhuma nova store: o
          próprio Studio executa a ação.
        </p>
        <textarea
          value={prompt}
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-4">
        <div className="text-xs uppercase tracking-widest text-muted-foreground">Comandos preparados</div>
