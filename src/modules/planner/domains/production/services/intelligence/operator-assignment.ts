...
function pickBest(candidates: readonly FactoryOperator[]): FactoryOperator | null {
  ...
  return [...candidates].sort((a, b) => (b.efficiency * 10 - b.loadH) - (a.efficiency * 10 - a.loadH))[0];
}

export function assignOperators(
  operators: readonly FactoryOperator[] = FACTORY_OPERATORS,
): readonly OperatorAssignment[] {
  const stages: RoutingStage[] = ["corte", "coladeira", "usinagem", "montagem", "conferencia", "embalagem", "expedicao"];
  const out: OperatorAssignment[] = [];
  for (const stage of stages) {
    const skill = STAGE_SKILL[stage];
    const best = pickBest(operators.filter((o) => o.skills.includes(skill) && o.status !== "afastado"));
    if (!best) continue;
    out.push({
      stage,
      operatorId: best.id,
