...
function pushOp(ops: DrillOp[], base: Omit<DrillOp, "id">): void {
  ...
  if (isCase) {
    ...
    for (const kind of minifixes) {
      pushOp(ops, {
        ...
        x: inset,
        y: inset,
        diameterMm: kind === "minifix-cabeca" ? 15 : 5,
        depthMm: kind === "minifix-cabeca" ? 12.5 : 34,
        tool: "broca-hss",
      });
      pushOp(ops, {
        partCode: code,
...
export function buildDrillingSheets(parts: readonly ProductionPart[]): readonly DrillingSheet[] {
...
export function countDrillOpsByKind(sheets: readonly DrillingSheet[]): Record<DrillOpKind, number> {
