...
export function buildBudget(input: BudgetInput): Budget {
  ...
  const lines: BudgetLine[] = [
    {
      id: "mat-chapa",
      group: "materiais",
      label: `Chapas (${input.cuttingPlan.totals.boardsCount} un.)`,
      qty: input.cuttingPlan.totals.boardsCount,
      unit: "chapa",
      unitPrice: DEFAULT_BOARD_PRICE,
      total: boardsTotal,
    },
    {
      id: "mat-fita",
      group: "materiais",
      label: "Fita de borda",
      qty: Math.round(edgeMeters),
      unit: "m",
      unitPrice: EDGE_TAPE_PRICE_M,
      total: Math.round(edgeTotal * 100) / 100,
    },
    {
      id: "fer-total",
      group: "ferragens",
      label: "Ferragens (consolidadas)",
      qty: 1,
      unit: "kit",
      unitPrice: hardwareTotal,
      total: Math.round(hardwareTotal * 100) / 100,
    },
    {
      id: "mo-marc",
      group: "mao-obra",
      label: "Marcenaria (h)",
      qty: Math.round(input.time.totalH * 10) / 10,
      unit: "h",
      unitPrice: params.laborRatePerHour,
      total: Math.round(laborTotal * 100) / 100,
    },
    {
      id: "pintura",
      group: "pintura",
      label: `Pintura/laca ${paintM2.toFixed(1)} m²`,
      qty: Math.round(paintM2 * 10) / 10,
      unit: "m²",
      unitPrice: PAINT_PRICE_M2,
      total: Math.round(paintTotal * 100) / 100,
    },
    {
      id: "montagem",
      group: "montagem",
      label: "Montagem no local",
      qty: 1,
      unit: "verba",
      unitPrice: assemblyTotal,
      total: assemblyTotal,
    },
    {
      id: "entrega",
      ...
      qty: 1,
      unit: "verba",
      unitPrice: params.deliveryFlat,
      total: params.deliveryFlat,
    },
  ];
  ...
  return {
    lines,
    summary: { subtotal, overhead, margin, taxes, final, perM2 },
    parameters: params,
  };
}
