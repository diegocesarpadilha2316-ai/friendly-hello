import type { PlanningKpis, PlanningOrder, MrpSummary, CapacityWindow, DeliveryEstimate } from "./types";
...
export function buildPlanningKpis(input: {
  ...
  const completed = orders.filter((o) => o.status === "concluido" || o.status === "entregue").length;
