...
export function getMaterial(id: string): LibraryMaterial | null {
  return getCachedLibraryMaterial(id);
}
export function requestMaterial(id: string): Promise<LibraryMaterial | null> {
  return requestLibraryMaterial(id);
}
export function resolveMaterials(ids: readonly string[]): Map<string, LibraryMaterial> {
  return resolveLibraryMaterialsSync(ids);
}
