...
export async function findMaterialByHint(hint: string): Promise<LibraryMaterial | null> {
  ...
  const filtered = thickness ? results.filter((m) => Math.abs(m.thicknessMm - thickness) < 0.5) : results;
...
export async function findManyMaterials(hint: string, limit = 12): Promise<readonly LibraryMaterial[]> {
  return searchLibraryMaterials({ query: normalize(hint), limit });
}
export async function findManyHardware(hint: string, limit = 12): Promise<readonly LibraryHardware[]> {
