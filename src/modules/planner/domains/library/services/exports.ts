...
export function exportMaterials(
  ...
  if (format === "json")
    return {
      ...
      filename: "biblioteca-materiais.json",
    };
  if (format === "xml")
    return {
      content: `<materials>\n${items.map((i) => `  <m id="${i.id}"/>`).join("\n")}\n</materials>`,
      mime: "application/xml",
      filename: "biblioteca-materiais.xml",
    };
  if (format === "excel")
    return {
      content: "\uFEFF" + csv(rows, H),
      mime: "text/csv;charset=utf-8",
      filename: "biblioteca-materiais.csv",
    };
  if (format === "bom")
    return {
      content: csv(rows, ["id", "name", "manufacturer", "thicknessMm", "pricePerM2"]),
      mime: "text/csv",
      filename: "biblioteca-materiais-bom.csv",
    };
  return { content: csv(rows, H), mime: "text/csv", filename: "biblioteca-materiais.csv" };
}

export function exportHardware(
  ...
  if (format === "json")
    return {
      content: JSON.stringify(items, null, 2),
      mime: "application/json",
      filename: "biblioteca-ferragens.json",
    };
  if (format === "xml")
    return {
      content: `<hardware>\n${items.map((i) => `  <h id="${i.id}"/>`).join("\n")}\n</hardware>`,
      mime: "application/xml",
      filename: "biblioteca-ferragens.xml",
    };
  if (format === "excel")
    return {
      content: "\uFEFF" + csv(rows, H),
      mime: "text/csv;charset=utf-8",
      filename: "biblioteca-ferragens.csv",
    };
  if (format === "bom")
    return {
      content: csv(rows, ["id", "model", "manufacturer", "unitPrice"]),
      mime: "text/csv",
      filename: "biblioteca-ferragens-bom.csv",
    };
