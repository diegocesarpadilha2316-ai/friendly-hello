 ...
 */
import { z } from "zod";
import { TOOL_FUNCTIONS, } from "../services/tools";
import { estimateBudget, preliminaryCutList, productionSummary, searchMaterialTool, } from "./advisory";
import { checkCirculation, reviewProject } from "./inspection";
import { getScenePrefs, setScenePrefs } from "./scene-prefs";
import { LIMITS, dimensionMm, pointMm, positiveInt, shapeEnum, shortText, wallEnum, } from "./validation";
...
function contract(name, meta) {
    return {
        ...
        timeout: meta.timeout ?? 8000,
        maxAffected: meta.maxAffected,
        execute: (meta.execute ?? legacy(name)),
    };
}
/* ----------------------------- schemas ----------------------------- */
...
function buildContracts() {
    return [
        ...
        contract("set_style", {
            description: "Harmoniza todo o cômodo em um estilo (minimalista, clássico, industrial, luxo, moderno).",
            owner: "designer",
            category: "layout",
            mutating: true,
        ...
        contract("review_project", {
            description: "Revisão completa e estruturada do projeto: limites, dimensões, colisões, materiais, identificação, aberturas, circulação e prontidão para orçamento/produção/render. Nunca corrige sozinha.",
            owner: "designer",
            category: "inspection",
            timeout: 12000,
            schema: empty(),
        ...
        contract("insert_described", {
            description: "Casa uma descrição livre com um item real do catálogo e insere com medidas e acabamento.",
            owner: "marceneiro",
            category: "furniture",
            mutating: true,
            maxAffected: 20,
        ...
        contract("search_material", {
            description: "Busca acabamentos REAIS no catálogo de chapas por nome e marca. Use antes de aplicar material — nunca invente acabamento.",
            owner: "materiais",
            category: "materials",
            requiresProject: false,
            schema: z.object({ query: shortText(120), brand: shortText(60).optional() }).strict(),
            execute: (args, run) => {
                const { query, brand } = args;
                void run;
        ...
        contract("set_module_params", {
            description: "Altera apenas os atributos citados do módulo (portas, abertura, gavetas, prateleiras, divisões, maleiro, cabideiros, nichos, espelho, puxador).",
            owner: "marceneiro",
            category: "furniture",
            mutating: true,
            schema: z
        ...
        contract("estimate_budget", {
            description: "Estimativa por categoria (chapas, fita, ferragens, serviços) a partir da decomposição real. Nunca inventa preço: o que não tem valor no catálogo é declarado como pendência.",
            owner: "orcamentista",
            category: "budget",
            timeout: 12000,
            schema: empty(),
        ...
        contract("production_summary", {
            description: "Resumo de produção: módulos, peças, área de chapa e metros de fita, com pendências.",
            owner: "producao",
            category: "production",
            timeout: 12000,
            schema: empty(),
        ...
        contract("preliminary_cut_list", {
            description: "Lista de corte PRELIMINAR derivada da decomposição real. Não substitui o plano de corte oficial.",
            owner: "producao",
            category: "production",
            timeout: 15000,
            schema: empty(),
        ...
        contract("set_render_preset", {
            description: "Define preset de cena (qualidade, iluminação, exposição) para a pré-visualização. Não persiste no projeto.",
            owner: "render",
            category: "render",
            schema: z
                .object({
        ...
        contract("set_camera", {
            description: "Ajusta a câmera da pré-visualização (altura do olho e abertura). Não persiste no projeto.",
            owner: "render",
            category: "render",
            schema: z
                .object({
...
export function contractOwner(name) {
