...
function contract(name: ToolName, meta: Meta): PlannerToolContract {
  return {
    ...
    execute: (meta.execute ??
      legacy(name)) as PlannerToolContract["execute"],
  };
}
...
function buildContracts(): readonly PlannerToolContract[] {
  ...
  contract("set_render_preset", {
    ...
    execute: (args, run) => {
      const a = args as { quality?: RenderQuality; lighting?: "natural" | "cenica" | "noturna" | "estudio"; exposure?: number };
