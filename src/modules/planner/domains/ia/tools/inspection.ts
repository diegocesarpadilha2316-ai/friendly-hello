...
function isDecor(f: FurniturePrimitive): boolean {
...
export interface CirculationReport {
  ...
  readonly freeAreaRatio: number;
  readonly minGapMm: number | null;
...
export interface ReviewReport {
  readonly findings: readonly ReviewFinding[];
  readonly counts: { info: number; warning: number; error: number };
  readonly note: string;
}

/** Revisão canônica do projeto — diagnóstico, nunca correção automática. */
export function reviewProject(project: PlannerProject, ctx: ToolContext): ReviewReport {
  const findings: ReviewFinding[] = [];
  const room = getActiveRoom(project, ctx);
