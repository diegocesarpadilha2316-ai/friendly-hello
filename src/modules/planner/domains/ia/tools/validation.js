 ...
 */
import { z } from "zod";
import { CATALOG_ITEMS, MATERIAL_BRANDS, findCatalogItem, listPrimitives, } from "@/modules/planner/shared";
...
export function toMillimeters(value) {
    if (typeof value === "number" && Number.isFinite(value)) {
        if (value !== 0 && Math.abs(value) <= 20)
            return Math.round(value * 1000); // metros
    ...
    if (unit.startsWith("mm") || unit.startsWith("mili"))
        return Math.round(raw);
    if (unit.startsWith("cm") || unit.startsWith("centi"))
        return Math.round(raw * 10);
    if (unit === "m" || unit.startsWith("metro"))
...
export function searchMaterials(query, brandFilter) {
    ...
    for (const brand of MATERIAL_BRANDS) {
        if (brandFilter && normalize(brand.label).indexOf(normalize(brandFilter)) < 0)
            continue;
