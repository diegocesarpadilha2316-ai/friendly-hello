...
export async function runPlannerTool(input) {
    ...
    if (contract.destructive && !input.confirmed) {
        return fail(input, agent, "NEEDS_CONFIRMATION", "Esta operação remove ou substitui conteúdo do projeto e precisa da sua confirmação explícita.");
    }
    ...
    try {
        const outcome = await Promise.race([
            Promise.resolve(contract.execute(parsed.data, {
                project: input.project,
                ctx: input.ctx,
        ...
        if (contract.maxAffected && affected.length > contract.maxAffected) {
            return fail(input, agent, "LIMIT_EXCEEDED", `A operação afetaria ${affected.length} objetos, acima do limite seguro de ${contract.maxAffected}.`);
        }
        ...
        return fail(input, agent, timedOut ? "TIMEOUT" : "INTERNAL", timedOut
