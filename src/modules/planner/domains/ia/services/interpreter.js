 ...
 */
const REAL_MODULE_NOUN = /armari|aereo|balcao|gaveteir|torre|cristaleir|roupeir|guarda[-\s]?roupa|closet|painel|nicho|prateleir|bancada|tampo|ilha|pia|cuba|coifa|cooktop|forno|geladeira/;
...
export function interpret(input) {
    ...
    if (!raw)
        return { type: "smalltalk", reply: "Como posso te ajudar no Planner?" };
    const t = norm(raw);
    // ── Perguntas ──
    if (t.startsWith("qual") ||
        ...
        if (has(t, "porta"))
            return { type: "question", question: { kind: "door_count" } };
        if (has(t, "gaveta"))
            return { type: "question", question: { kind: "drawer_count" } };
        if (has(t, "ferragem", "puxador", "dobradica", "corredica"))
            return { type: "question", question: { kind: "hardware" } };
        if (has(t, "peca", "pecas"))
            return { type: "question", question: { kind: "part_count" } };
    ...
    const ambientWords = has(t, "projeto", "ambiente", "cozinha", "closet", "dormitorio", "quarto", "sala", "estar", "living", "escritorio", "home office", "banheiro", "lavabo", "lavanderia", "completo", "completa", "inteir", "todo", "toda");
    {
        ...
        if (!matchedPreset && !ambientWords && wantsInsertVerb) {
            ...
            if (dec.modules.length > 0) {
                ...
                for (const m of dec.modules) {
                    const alreadyHasFinish = /(freijo|nogueira|carvalho|branco|preto|grafite|chumbo|off\s*white|quartzo|cumaru|louro)/i.test(m.description);
                    const description = material && !alreadyHasFinish ? `${m.description} ${material}` : m.description;
                    intents.push({
                        tool: "insert_described",
    ...
    // ── Ações unitárias ──
    if (has(t, "duplique", "duplicar", "duplica"))
        intents.push({ tool: "duplicate", args: {} });
    if (has(t, "espelhe", "espelhar", "espelha"))
        intents.push({ tool: "mirror", args: {} });
    ...
    if (has(t, "centralize", "centralizar", "centraliza"))
        intents.push({ tool: "center", args: {} });
    // ── Bancada / tampo ──
    ...
    return {
        type: "unknown",
        reply: "Ainda não entendi esse comando. Tente: 'crie uma cozinha moderna', 'troque para Freijó', 'abra todas as portas', 'centralize este armário'.",
    };
}
