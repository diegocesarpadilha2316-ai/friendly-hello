...
function toMm(value, unit) {
    ...
    if (value < 10)
        return Math.round(value * 1000); // 2,70 → 2700
    if (value < 100)
        return Math.round(value * 10); // 60 → 600
    return Math.round(value);
}
function dimByLabel(t, labels) {
...
export function countOf(t, kind) {
...
// ───────────────────────── construção da ficha ─────────────────────────
export function buildFurnitureSpec(input, hint) {
    const t = norm(input);
    ...
    if (envMatch || hint?.environment)
        explicit.add("environment");
    if (!environment) {
        environment = type ? (TYPE_WORDS.find((x) => x.id === type)?.env ?? "cozinha") : "cozinha";
        assumptions.push(`ambiente ${environment}`);
    }
    // 3) medidas
    const seq = t.match(/(\d+(?:[.,]\d+)?)\s*(?:mm|cm|m)?\s*(?:x|×)\s*(\d+(?:[.,]\d+)?)\s*(?:mm|cm|m)?(?:\s*(?:x|×)\s*(\d+(?:[.,]\d+)?))?\s*(mm|cm|m)?/);
    ...
    if (seq && width == null && height == null) {
        ...
        if (seq[3])
            depth = toMm(Number(seq[3].replace(",", ".")), unit);
    }
    if (width != null)
        explicit.add("width");
    else
        assumptions.push(`largura ${base.width} mm`);
    if (height != null)
        explicit.add("height");
    else
        assumptions.push(`altura ${base.height} mm`);
    if (depth != null)
        explicit.add("depth");
    else
        assumptions.push(`profundidade ${base.depth} mm`);
    // 4) portas / abertura
    let doors = countOf(t, "porta");
    if (/porta\s*dupla/.test(t))
        doors = doors ?? 2;
    if (/porta\s*(simples|unica)/.test(t))
        doors = doors ?? 1;
    if (doors != null)
        explicit.add("doors");
    const openingMatch = OPENING_WORDS.find((o) => o.re.test(t));
    let opening = openingMatch?.opening ?? base.opening;
    if (openingMatch)
        explicit.add("opening");
    if ((doors ?? base.doors) === 0)
        opening = "sem-porta";
    // 5) gavetas / divisões / prateleiras
    const drawers = countOf(t, "gaveta");
    if (drawers != null)
        explicit.add("drawers");
    const shelves = countOf(t, "prateleir");
    if (shelves != null)
        explicit.add("shelves");
    const divisions = countOf(t, "(?:divis|modul)");
    if (divisions != null)
        explicit.add("divisions");
    ...
    if (materialMatch)
        ...
        assumptions.push("chapa MDF 18 mm");
    const color = colorMatch?.color ?? "Louro Freijó";
    if (colorMatch)
        explicit.add("color");
    else
        assumptions.push("acabamento Louro Freijó");
    const style = styleMatch?.style ?? "moderno";
    if (styleMatch)
        explicit.add("style");
    else
        assumptions.push("estilo moderno");
    const handle = handleMatch?.handle ?? (style === "classico" ? "botao" : "perfil-gola");
    if (handleMatch)
        explicit.add("handle");
    else
        assumptions.push(handle === "botao" ? "puxador botão" : "puxador perfil gola");
    // 8) acessórios
    const accessories = ACCESSORY_WORDS.filter((a) => a.re.test(t)).map((a) => a.label);
    if (accessories.length > 0)
        explicit.add("accessories");
    // 9) posição
    let position = "bottom";
    if (/parede\s*(?:da\s*)?esquerda|lado\s*esquerdo|a\s*esquerda/.test(t))
        position = "left";
    else if (/parede\s*(?:da\s*)?direita|lado\s*direito|a\s*direita/.test(t))
        position = "right";
    else if (/parede\s*(?:do\s*)?fundo|ao\s*fundo|no\s*fundo/.test(t))
        position = "top";
    else if (/no\s*centro|centraliz/.test(t))
        position = "centro";
    if (position !== "bottom")
 ...
 */
export function specToDescription(spec) {
    const bits = [];
    if (spec.type)
        bits.push(TYPE_LABEL[spec.type]);
    if (spec.doors > 0)
        bits.push(`${spec.doors} portas`);
    if (spec.opening === "correr")
        bits.push("de correr");
    if (spec.drawers > 0)
        bits.push(`${spec.drawers} gavetas`);
    if (spec.mirror?.has)
        bits.push("espelho");
    if (spec.cabideiros > 0)
        bits.push("cabideiro");
    if (spec.accessories.includes("Sapateira"))
        bits.push("sapateira");
...
export function specToParams(spec) {
    ...
    if (spec.divisions > 0)
        params["mod:divisions"] = spec.divisions;
    if (spec.maleiro)
        params["mod:maleiro"] = true;
    if (spec.cabideiros > 0)
        params["mod:cabideiros"] = spec.cabideiros;
    if (spec.nichos > 0)
        params["mod:nichos"] = spec.nichos;
    if (spec.mirror?.has) {
        params["mod:mirror"] = true;
        params["mod:mirrorPosition"] = spec.mirror.position;
    }
    if (spec.opening === "sem-porta")
        params["eng:front"] = "aberto";
    if (spec.accessories.length > 0)
        params["mod:accessories"] = spec.accessories.join(", ");
    return params;
}
/** Resumo humano da ficha (auditoria / debug). */
export function describeSpec(spec) {
    if (!spec.type)
        return "ficha incompleta (tipo ausente)";
    const parts = [
    ...
    if (spec.maleiro)
        parts.push("maleiro");
    if (spec.cabideiros > 0)
        parts.push(`${spec.cabideiros} cabideiro(s)`);
    if (spec.nichos > 0)
        parts.push(`${spec.nichos} nicho(s)`);
    if (spec.mirror?.has)
        parts.push(`espelho (${spec.mirror.position})`);
    parts.push(spec.color, spec.material, spec.style);
    return parts.join(" · ");
}
/** Frase curta com as suposições, para a IA informar no chat. */
export function assumptionsSentence(spec) {
