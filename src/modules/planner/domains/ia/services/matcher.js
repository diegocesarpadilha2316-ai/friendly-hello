 ...
 */
import { CATALOG_ITEMS, findCatalogItem, } from "@/modules/planner/shared";
...
function detectSubtype(text) {
    ...
    if (/balcao\s+(?:d[ea]\s+)?pia|balcao\s+pia|balcao.*cuba/.test(text))
        return "balcao";
    if (/armari[oa]\s+(?:sob\s+)?(?:d[ea]\s+)?pia|armari[oa].*cuba/.test(text))
        return "armario";
    for (const h of SUBTYPE_HINTS)
        if (h.words.some((w) => text.includes(w)))
            return h.subtype;
    for (const h of DECOR_HINTS)
        if (h.words.some((w) => text.includes(w)))
            return h.subtype;
    return null;
}
...
function parseDimensionByLabel(text, labels) {
...
function clampParam(value, range) {
...
// ─────────── frente ───────────
function detectFront(text) {
    if (/(reeded|canelad|ripad(o|a)\s*em\s*vidro)/.test(text))
        return "reeded";
    if (/(porta\s+de\s+vidro|em\s+vidro|com\s+vidro|vidro)/.test(text))
        return "vidro";
    if (/(sem\s+porta|aberto|prateleir[ao]s?\s+aberta)/.test(text))
        return "aberto";
    if (/(porta\s+solida|frente\s+solida|solid[ao])/.test(text))
        return "solid";
    return null;
}
// ─────────── cor / madeira ───────────
...
function detectMaterial(text) {
    if (/mdp/.test(text))
        return "MDP 18mm";
    if (/compensad/.test(text))
        return "Compensado 18mm";
    if (/mdf\s*15/.test(text))
        return "MDF 15mm";
    if (/mdf/.test(text))
        return "MDF 18mm";
    return null;
...
function detectDoorCount(text) {
    ...
    if (/porta\s*(simples|unica|single)/.test(text))
        return 1;
    if (/porta\s*dupla/.test(text) || /duas\s*portas/.test(text))
        return 2;
    if (/tres\s*portas|porta\s*tripla/.test(text))
        return 3;
    if (/quatro\s*portas/.test(text))
        return 4;
    return null;
}
function detectDrawerCount(text) {
    ...
    if (/duas\s*gavetas/.test(text))
        return 2;
    if (/tres\s*gavetas/.test(text))
        return 3;
    if (/quatro\s*gavetas/.test(text))
        return 4;
    if (/cinco\s*gavetas/.test(text))
        return 5;
    if (/seis\s*gavetas/.test(text))
        return 6;
    return null;
}
// ─────────── seleção do item ───────────
...
function semanticPriority(c, subtype, text) {
    ...
    if (wantsSink) {
        if (subtype === "balcao" && (id === "balcao-gourmet" || haystack.includes("cuba")))
            s += 1800;
        if (subtype === "armario" &&
            (id.startsWith("arm-sob-pia") || haystack.includes("sob pia") || tags.includes("pia")))
            s += 1800;
        if (!haystack.includes("pia") && !haystack.includes("cuba"))
            s -= 500;
    }
    if (/\bcooktop\b/.test(text) && haystack.includes("cooktop"))
        s += 1200;
    if (/\bforno\b|micro-?ondas/.test(text) &&
        (haystack.includes("forno") || haystack.includes("micro")))
        s += 1200;
    if (/\bbar\b/.test(text) && haystack.includes("bar"))
        s += 900;
    return s;
}
function pickBestItem(subtype, wantedWidth, text, wantedDoors, wantedDrawers) {
 ...
 */
export function matchDescription(description, fallback) {
    const t = norm(description);
    const subtype = detectSubtype(t) ?? fallback?.subtype ?? null;
    let item = null;
    if (fallback?.catalogItemId)
        item = findCatalogItem(fallback.catalogItemId) ?? null;
    ...
    if (!item && subtype)
        item = pickBestItem(subtype, wantedWidth, t, wantedDoors, wantedDrawers);
    if (!item)
        return null;
    const overrides = {};
    const reasons = [`item: ${item.name}`];
    if (wantedWidth != null) {
    ...
    if (/\bpia\b|\bcuba\b|sob\s+pia/.test(t) ||
