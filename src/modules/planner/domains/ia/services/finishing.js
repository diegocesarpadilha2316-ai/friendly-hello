...
export function findFinishingPreset(id) {
    ...
    return (FINISHING_PRESETS.find((p) => p.id === norm) ??
...
export function applyFinishing(project, target, args) {
    ...
    if (!preset)
        return { error: `Preset "${args.presetId}" não encontrado.` };
    const scope = args.scope ?? "all";
    let applied = 0;
    const environments = project.environments.map((env) => {
        if (env.id !== target.environmentId)
            return env;
        return {
            ...env,
            rooms: env.rooms.map((room) => room.id !== target.roomId
                ? room
