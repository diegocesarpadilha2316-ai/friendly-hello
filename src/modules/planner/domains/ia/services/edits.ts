/**
 * Edições cirúrgicas — "altere só o que foi pedido".
 *
 * Converte comandos de alteração em pt-BR ("troque as portas por portas de
 * correr", "aumente a largura para 3 metros", "coloque 4 gavetas internas")
 * em intents que tocam EXCLUSIVAMENTE os atributos citados. Nada mais do
 * projeto é recriado, reposicionado ou substituído.
 *
 * Determinístico, puro e offline. Consumido pelo interpretador antes de
 * qualquer rota de criação.
 */
...
export interface EditIntent {
  ...
  /** Descrição humana do que foi alterado (para o chat). */
  change: string;
}
...
function dimension(t: string, labels: readonly string[]): number | null {
  const label = labels.join("|");
  const re = new RegExp(
    `(?:${label})\\s*(?:para|pra|em|de|a)?\\s*(\\d+(?:[.,]\\d+)?)\\s*(mm|cm|m)?`,
  );
...
const COLORS: Array<{ color: string; re: RegExp }> = [
...
const HANDLES: Array<{ handle: string; label: string; re: RegExp }> = [
 ...
 */
export function parseEdits(input: string): EditIntent[] {
  const t = norm(input);
  if (!EDIT_VERB.test(t)) return [];
  if (CREATE_VERB.test(t)) return [];
