...
import { buildAgentPlan, describeAgents, selectConversationalAgents, startAgentRun, } from "../agents";
/**
...
async function* runCommand(intents, input) {
    ...
    const closeRun = (success, error) => {
        if (handle)
            handle.finish(success, [...agentTools], error);
        handle = null;
        agentTools.length = 0;
    };
    for (const step of plan.steps) {
        ...
        let outcome;
        try {
            const executed = await executeIntent(intent, project, input, `${turnId}:${plan.steps.indexOf(step)}:${step.tool}`);
