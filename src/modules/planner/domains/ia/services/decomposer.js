...
function chunkIsOnlyPreamble(chunk) {
    const words = chunk
        .replace(/[.,;:!?]/g, " ")
        .split(/\s+/)
        .filter(Boolean);
...
function extractWall(text) {
    if (/parede\s+(?:da\s+)?esquerda|lado\s+esquerdo|a\s+esquerda/.test(text))
        return "left";
    if (/parede\s+(?:da\s+)?direita|lado\s+direito|a\s+direita/.test(text))
        return "right";
    if (/parede\s+(?:do\s+)?fundo|ao\s+fundo|no\s+fundo/.test(text))
        return "top";
    if (/parede\s+(?:frontal|da\s+frente)|na\s+frente|frontal/.test(text))
        return "bottom";
    return undefined;
}
function extractFinish(text) {
    if (/louro\s+freijo|louro-freijo/.test(text))
        return "Louro Freijó";
    if (/freijo/.test(text))
        return "Freijó";
    if (/preto\s+absoluto|preto|preta/.test(text))
        return "Preto Absoluto";
    if (/branco\s+tx|branco|branca/.test(text))
        return "Branco TX";
    if (/off\s+white|off-white/.test(text))
        return "Off White";
    if (/nogueira/.test(text))
        return "Nogueira";
    if (/carvalho/.test(text))
        return "Carvalho";
    if (/grafite|chumbo/.test(text))
        return "Grafite";
    return undefined;
}
function chunkIsOnlyQualifier(chunk) {
    if (chunkIsOnlyPreamble(chunk))
        return true;
    const cleaned = chunk
...
function extractNumber(chunk, kind) {
    ...
    if (kind === "porta") {
        if (/porta\s*dupla|duas\s*portas/i.test(chunk))
            return 2;
        if (/tres\s*portas|porta\s*tripla/i.test(chunk))
            return 3;
        if (/quatro\s*portas/i.test(chunk))
            return 4;
    }
    else {
        if (/duas\s*gavetas/i.test(chunk))
            return 2;
        if (/tres\s*gavetas/i.test(chunk))
            return 3;
        if (/quatro\s*gavetas/i.test(chunk))
            return 4;
        if (/cinco\s*gavetas/i.test(chunk))
            return 5;
        if (/seis\s*gavetas/i.test(chunk))
            return 6;
    }
    return undefined;
}
function extractCount(chunk) {
...
export function decompose(input) {
    ...
    for (const chunk of chunks) {
        ...
        if (!matched) {
            ...
            if (/gaveta/.test(chunk))
                matched = TOKENS.find((x) => x.id === "gaveteiro") ?? null;
            else if (/porta/.test(chunk))
                matched = TOKENS.find((x) => x.id === "aereo") ?? null;
        }
        if (!matched) {
            if (chunk.length > 2 && !chunkIsOnlyQualifier(chunk))
        ...
        const bits = [matched.descBase];
        if (width)
            bits.push(`${width}mm largura`);
        if (height)
            bits.push(`${height}mm altura`);
        if (depth)
            bits.push(`${depth}mm profundidade`);
        if (doors)
            bits.push(`${doors} portas`);
        if (drawers)
            bits.push(`${drawers} gavetas`);
        if (globalFinish)
            bits.push(globalFinish);
        modules.push({
            raw: chunk,
            label: matched.label,
...
export function toLayoutPieces(dec) {
...
export function validateComposition(dec, placedLabels) {
    ...
    for (const m of dec.modules) {
        ...
        if (found < m.count)
            missing.push(`${m.label} (${found}/${m.count})`);
        if (m.doors)
            notes.push(`${m.label}: ${m.doors} porta(s) solicitadas`);
        if (m.drawers)
            notes.push(`${m.label}: ${m.drawers} gaveta(s) solicitadas`);
    }
    return {
        ok: missing.length === 0,
        requested,
