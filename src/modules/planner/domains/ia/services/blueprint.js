...
function detectMaterial(t) {
    for (const m of MATERIAL_WORDS)
        if (m.words.some((w) => t.includes(w)))
            return m.material;
    return undefined;
}
/**
...
function inferEnvironment(t) {
    ...
    if (/(closet)/.test(t))
        return "closet";
    if (/(quarto|dormit)/.test(t))
        return "dormitorio";
    if (/(sala|living|estar)/.test(t))
        return "sala";
    if (/(escritori|home office)/.test(t))
        return "escritorio";
    if (/(banheir|lavab)/.test(t))
        return "banheiro";
    if (/(lavanderi|area de servico)/.test(t))
        return "lavanderia";
    return null;
}
/**
...
function inferFromModules(dec) {
    ...
    if (labels.some((l) => l.includes("roupeir")))
        return "dormitorio";
    if (labels.some((l) => l.includes("closet")))
        return "closet";
    if (labels.some((l) => l.includes("painel")))
        return "sala";
    if (labels.some((l) => l.includes("espelho") || l.includes("nicho")))
        return "banheiro";
    return null;
}
// ─────────────────────────────────────────────────────────────────────────
...
export function validateBlueprint(bp) {
    ...
    for (const m of bp.modules) {
        if (m.count < 1 || m.count > 20)
            errors.push(`${m.label}: quantidade inválida (${m.count}).`);
...
export function blueprintToPreset(bp) {
    ...
    const hasFinish = (d) => /(freijo|nogueira|carvalho|branco|preto|grafite|chumbo|off\s*white|quartzo|cumaru|louro)/i.test(d);
    const pieces = bp.modules.map((m) => ({
        description: material && !hasFinish(m.description) ? `${m.description} ${material}` : m.description,
        count: m.count,
        wall: m.wall,
        width: m.width,
        height: m.height,
...
export function auditBlueprint(project, bp) {
    ...
    for (const m of bp.modules) {
        ...
        if (m.doors)
            notes.push(`${m.label}: ${m.doors} porta(s)`);
        if (m.drawers)
            notes.push(`${m.label}: ${m.drawers} gaveta(s)`);
    }
    return {
        ok: missing.length === 0,
...
export function describeBlueprint(bp) {
    ...
    if (bp.style)
        parts.push(`estilo=${bp.style}`);
    if (bp.material)
        parts.push(`material=${bp.material}`);
