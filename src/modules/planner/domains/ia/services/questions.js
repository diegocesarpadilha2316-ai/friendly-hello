...
export function answerQuestion(q, project, ctx, rules) {
    ...
    if (!room)
        return "Selecione um cômodo primeiro para eu responder.";
    ...
    switch (q.kind) {
        ...
        case "materials": {
            if (items.length === 0)
                return "Ainda não há móveis para analisar.";
        ...
        case "part_count": {
            let total = 0;
            for (const f of items)
                total += decomposeFurniture(f, rules).totals.partCount;
        ...
        case "board_area": {
            let m2 = 0;
            for (const f of items)
                m2 += decomposeFurniture(f, rules).totals.boardAreaM2;
