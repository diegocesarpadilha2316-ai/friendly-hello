/**
 * Etapa 11 — Parte 16: relatório final estruturado do plano.
 *
 * Derivado exclusivamente do resultado real das etapas (nada inventado).
 */
import type { PlanFinalReport, PlanStatus, ProjectPlan } from "./types";

function objectsFrom(plan: ProjectPlan, kinds: readonly string[]): string[] {
  const out = new Set<string>();
  for (const step of plan.steps) {
    if (step.status !== "completed") continue;
    if (!kinds.some((k) => step.toolName.startsWith(k))) continue;
    for (const id of step.result?.affectedIds ?? []) out.add(id);
  }
  return [...out];
}

export function buildFinalReport(plan: ProjectPlan, status: PlanStatus): PlanFinalReport {
  ...
  const has = (needle: string) => plan.steps.some((s) => s.toolName.includes(needle));
  const done = (needle: string) => completed.some((s) => s.toolName.includes(needle));
