...
const FULL_PROJECT = /\b(cozinha|closet|quarto|dormitorio|escritorio|home ?office|sala|banheiro|lavabo|area gourmet|varanda|painel de tv|home theater)\b/;
const FULL_HINT = /\b(complet\w+|inteir\w+|projet\w+|monte|montar|criar?|crie|faca|fazer|planeje|planejar|reforme|do inicio)\b/;
const INTERMEDIATE = /\b(monte|montar|organize|organizar|reorganize|distribua|planeje|proponha|layout|marcenaria|conjunto|bancada com|parede)\b/;
/** Pedido genérico ("crie um ambiente bonito") — falta o tipo de ambiente. */
const VAGUE_AMBIENT = /\b(ambiente|espaco|comodo|projeto)\b/;
const QUERY = /\b(qual|quais|quanto|quantos|como esta|esta aplicado|me diga|mostre|liste|existe|tem )\b/;
const SMALL_CHANGE = /\b(troque|trocar|mude|mudar|altere|alterar|ajuste|ajustar|pinte|pintar|aumente|diminua|renomeie)\b/;
...
export function detectRoomType(message) {
    return norm(message).match(FULL_PROJECT)?.[0] ?? null;
}
export function classifyRequest(message) {
    const t = norm(message);
    if (!t)
        return { kind: "conversa", needsPlan: false, reason: "mensagem vazia" };
