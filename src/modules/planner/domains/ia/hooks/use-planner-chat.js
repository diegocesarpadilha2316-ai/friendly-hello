...
export function usePlannerChat() {
  ...
  const sessionStorageKey = useCallback(
    (projectId) => `dioris.planner.ai.session.${tenantId}.${projectId ?? "sem-projeto"}`,
    [tenantId],
  );
  ...
  const persistMessage = useCallback(
    async (sessionId, role, content, status, clientMessageId) => {
          ...
          llmPlan: async ({ userMessage, project: p, ctx }) => {
            try {
              ...
              const catalog = listToolContracts()
                .map(
                  (t) =>
                    `- ${t.name} [${t.ownerAgent}${t.mutating ? "" : ", consultiva"}${t.destructive ? ", destrutiva" : ""}]: ${t.description}`,
                )
                .join("\n");
              const system =
              ...
              return intents
                ...
                .map((i) => ({
                  tool: i.tool,
                  args: i.args && typeof i.args === "object" ? i.args : {},
                }));
            } catch {
              return null;
            }
