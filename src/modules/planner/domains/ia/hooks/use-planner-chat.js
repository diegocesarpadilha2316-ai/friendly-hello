...
import { appendAiMessage, createAiSession, getAiSession, listAiSessions, recordAiToolCall, } from "@/lib/planner-ai.functions";
import { saveProjectSnapshot } from "@/lib/planner-snapshots.functions";
import { createEnvironment, createProject, ensureProjectRoomShells, createRoom, upsertProject, usePlannerEditor, loadRules, } from "@/modules/planner/shared";
...
function buildPlannerSystemPrompt(p, ctx, memoryBlock = "") {
...
function buildPlannerNoContextSystemPrompt(project) {
...
function inferRoomType(prompt) {
    const normalized = normalizePrompt(prompt);
    if (normalized.includes("cozinha"))
        return "cozinha";
    if (normalized.includes("closet"))
        return "closet";
    if (normalized.includes("banheiro") || normalized.includes("lavabo"))
        return "banheiro";
    if (normalized.includes("quarto") || normalized.includes("dormitorio"))
        return "dormitorio";
    if (normalized.includes("sala") || normalized.includes("living"))
        return "sala";
    if (normalized.includes("escritorio") || normalized.includes("home office"))
        return "escritorio";
    if (normalized.includes("lavanderia"))
        return "lavanderia";
    return "cozinha";
}
function labelForRoomType(type) {
    const labels = {
        closet: "Closet",
...
function ensureOperablePlannerContext(current, selectedEnvironmentId, selectedRoomId, meta) {
    const roomType = inferRoomType(meta.prompt);
    const fallbackProject = current ??
        createProject({
...
export function usePlannerChat() {
    ...
    const sessionStorageKey = useCallback((projectId) => `dioris.planner.ai.session.${tenantId}.${projectId ?? "sem-projeto"}`, [tenantId]);
    const readStoredSession = useCallback((projectId) => {
    ...
    const storeSession = useCallback((projectId, sessionId) => {
        if (typeof window === "undefined")
            return;
        try {
     ...
     */
    const ensureSession = useCallback(async (projectId, title) => {
        if (!activeCompany?.id)
            return null;
    ...
    const persistMessage = useCallback(async (sessionId, role, content, status, clientMessageId) => {
    ...
    useEffect(() => {
        if (!activeCompany?.id || !projectId || !isUuid(projectId))
            return;
        ...
        void (async () => {
            try {
                ...
                if (!latest || cancelled)
                    return;
                const detail = (await getSessionOnServer({ data: { id: latest.id, limit: 50 } }));
                if (cancelled)
                    return;
    ...
    const loadMore = useCallback(async () => {
        ...
        if (!session || !history.hasMore || !history.cursor || history.loading)
            return;
    ...
    const send = useCallback(async (text) => {
        ...
        if (!trimmed || state.status === "streaming" || state.status === "thinking")
            return;
        ...
        const boot = ensureOperablePlannerContext(editor.state.project, editor.state.selectedEnvironmentId, editor.state.selectedRoomId, { prompt: trimmed, tenantId, ownerId });
        const startedWithoutProject = !editor.state.project;
        const shouldOpenEditorAfterCreate = startedWithoutProject || pathname.endsWith("/planner/projetos/novo");
        ...
        if (boot.changed || operableProject !== boot.project) {
            if (editor.state.project)
                editor.updateProject(() => operableProject);
            else
                editor.loadProject(operableProject);
        ...
        if (activePlan && activePlan.status === "awaiting_confirmation") {
            const yes = /^\s*(sim|s|pode|confirmo?|confirmar|isso|ok|claro|manda|continuar?)\b/i.test(trimmed);
        ...
        if (classification.needsPlan && !planBusy) {
            ...
            if (proposed) {
                ...
                const content = proposed.status === "ready"
        ...
        try {
                ...
                llmPlan: async ({ userMessage, project: p, ctx }) => {
                    try {
                        ...
                        const catalog = listToolContracts()
                            .map((t) => `- ${t.name} [${t.ownerAgent}${t.mutating ? "" : ", consultiva"}${t.destructive ? ", destrutiva" : ""}]: ${t.description}`)
                            .join("\n");
                        const system = 'Você é o planejador do Dioris Planner. Traduza o pedido do usuário em uma sequência de chamadas de ferramentas (tool-calling). Responda SOMENTE com JSON válido no formato { "intents": [{ "tool": string, "args": object }] }. Não inclua explicações. Use apenas ferramentas da lista.\n\nUNIDADE OBRIGATÓRIA: todas as medidas em MILÍMETROS inteiros (800 = 800 mm; 1,20 m = 1200). Nunca envie centímetros ou metros como número cru.\nNunca invente material, acabamento ou preço: use search_material antes de aplicar acabamento e estimate_budget para valores.\nFerramentas destrutivas (remove) só devem ser planejadas quando o usuário pedir explicitamente.\n\nFerramentas disponíveis:\n' +
                            catalog +
                        ...
                        if (!Array.isArray(intents))
                            return null;
                        return intents
                            .filter((i) => !!i && typeof i === "object" && typeof i.tool === "string")
                            .map((i) => ({
                            tool: i.tool,
                            args: i.args && typeof i.args === "object"
                                ? i.args
                                : {},
                        }));
                    }
            ...
            if (mutatedProject !== project) {
                ...
                if (hasLoadedProject)
                    ...
                    editor.loadProject(mutatedProject);
            ...
            if (sessionId) {
                const assistantMessageId = await persistMessage(sessionId, "assistant", buffer || "(sem conteúdo)", "ok", keys.assistant);
            ...
            await persistMessage(sessionId, "assistant", "Resposta não concluída (falha no provedor).", "error", keys.assistant);
            sendingRef.current = false;
        }
    }, [
        state.status,
