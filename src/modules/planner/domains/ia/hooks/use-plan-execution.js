...
export function usePlanExecution(tenantId) {
    ...
    const buildRunner = useCallback((base, ctx) => {
    ...
    const propose = useCallback((input) => {
        ...
        if (generated.status === "ready") {
            void runner
                ...
                setPlan((prev) => prev ? { ...prev, status: "failed", warnings: [...prev.warnings, message] } : prev);
            });
        }
        return generated;
    }, [buildRunner, commit, tenantId]);
    const finishMemory = useCallback((finished) => {
        // Memória só é atualizada quando o plano termina de fato.
        if (finished.status !== "completed" && finished.status !== "partially_completed")
            return;
    ...
    const runNow = useCallback((mode) => {
        const runner = ensureRunner();
        if (!runner)
            return;
        const exec = mode === "resume"
            ? runner.resume()
            : mode === "retry"
                ? runner.retryFailed()
                : mode === "restart"
    ...
    const answerAndExecute = useCallback((answer) => {
    ...
    const dismiss = useCallback(() => {
        ...
        setPlan(null);
        if (current)
            writeStoredPlan(null, tenantId, current.projectId);
    }, [plan, tenantId]);
    return {
        plan,
