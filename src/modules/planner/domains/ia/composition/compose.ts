/**
 * COMPOSICAO DOS VOLUMES.
 *
 * Nao "preenche espaco": distribui os modulos pensando em equilibrio
 * visual, proporcao, alinhamento, continuidade de volumes, ergonomia e
 * funcionalidade. Devolve as MESMAS pecas aceitas por `applyLayout`,
 * apenas com parede/ordem decididas por regra de projeto.
 */
import type { LayoutShape, LayoutWall } from "../services/layout";
import type { ComposedPiece, CompositionResult, RoomAnalysis } from "./types";
...
/** Classe funcional do modulo — dirige altura, ancoragem e vizinhanca. */
export type PieceClass = "alto" | "base" | "aereo" | "eletro" | "apoio";

export function classifyPiece(description: string): PieceClass {
...
function visualWeight(cls: PieceClass): number {
  switch (cls) {
    case "alto":
      return 3;
    case "eletro":
...
function estimateWidth(p: ComposedPiece, cls: PieceClass): number {
...
const CLASS_ORDER: Record<PieceClass, number> = {
  alto: 0,
  eletro: 1,
  base: 2,
  apoio: 3,
...
export function composeLayout(
  ...
  const budgets = new Map<LayoutWall, WallBudget>();
  ...
  const pick = (cls: PieceClass, width: number, requested?: LayoutWall): LayoutWall => {
    const prefer: LayoutWall[] = [];
  ...
  const out: ComposedPiece[] = [];
  const aereoWallByBase = new Map<string, LayoutWall>();
  ...
  const usedWalls = new Set(out.map((p) => p.wall).filter(Boolean) as LayoutWall[]);
...
function promoteShape(base: LayoutShape, wallsUsed: number): LayoutShape {
