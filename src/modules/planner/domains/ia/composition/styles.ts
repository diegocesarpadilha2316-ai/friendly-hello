/**
 * Perfis de estilo — governam densidade, paleta de decoracao e o repertorio
 * de objetos permitidos. Toda a composicao (moveis + decor) obedece a UM
 * unico perfil por ambiente, garantindo coerencia estetica.
 */
import type { CompositionStyle, DecorRole } from "./types";

export interface StyleProfile {
  style: CompositionStyle;
  /** multiplicador de densidade decorativa (1 = referencia) */
  density: number;
  /** folga entre modulos, em mm — estilos limpos respiram mais */
  gap: number;
  /** repertorio permitido por papel decorativo (ordem = preferencia) */
  pools: Record<DecorRole, readonly string[]>;
  /** vocabulario que a IA usa para justificar decisoes */
  language: string;
}
...
const P = (
  ...
  pools: Record<DecorRole, readonly string[]>,
  language: string,
): StyleProfile => ({ style, density, gap, pools, language });

export const STYLE_PROFILES: Readonly<Record<CompositionStyle, StyleProfile>> = {
  moderno: P(
    "moderno",
    1,
    20,
...
const STYLE_ALIASES: Array<{ style: CompositionStyle; words: readonly string[] }> = [
...
export function resolveStyle(raw?: string | null): CompositionStyle {
...
export function styleProfile(style: CompositionStyle): StyleProfile {
