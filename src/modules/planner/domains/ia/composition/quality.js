...
function quadrantOf(x, y, a) {
    return (x < a.width / 2 ? 0 : 1) + (y < a.depth / 2 ? 0 : 2);
}
/** Mede a composicao final (moveis + decoracao) e aponta problemas. */
export function evaluateComposition(analysis, furniture, decor) {
    const issues = [];
    // ── Equilibrio: massa distribuida entre quadrantes ───────────────────
    const mass = [0, 0, 0, 0];
    for (const r of furniture) {
        mass[quadrantOf(r.x + r.w / 2, r.y + r.d / 2, analysis)] += (r.w * r.d) / 1000000;
    }
    for (const d of decor)
        mass[quadrantOf(d.x, d.y, analysis)] += 0.25;
    ...
    const occupiedM2 = furniture.reduce((s, r) => s + (r.w * r.d) / 1000000, 0);
    ...
    const blocking = furniture.filter((r) => r.x < corridor.x + corridor.w &&
        r.x + r.w > corridor.x &&
        r.y < corridor.y + corridor.d &&
        r.y + r.d > corridor.y).length;
    ...
    const score = Math.round(balance * 0.25 +
        proportion * 0.2 +
        circulation * 0.25 +
        organization * 0.15 +
        coherence * 0.15);
 ...
 */
export function rebalanceComposition(analysis, furniture, decor) {
    ...
    while (!report.ok && passes < 3 && current.length > 2) {
        ...
        for (const d of current)
            load[quadrantOf(d.x, d.y, analysis)] += 1;
