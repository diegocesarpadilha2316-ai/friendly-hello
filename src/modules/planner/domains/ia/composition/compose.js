...
export function classifyPiece(description) {
    ...
    if (/(torre|roupeir|closet|armario alto|despenseir|estante|panelei)/.test(t))
        return "alto";
    if (/(aereo|aéreo|basculante|suspens)/.test(t))
        return "aereo";
    if (/(geladeir|cooktop|forno|coifa|micro|lava|cook|fogao)/.test(t))
        return "eletro";
    if (/(balcao|balcão|gaveteir|bancada|pia|cuba|painel|rack|cristaleir)/.test(t))
        return "base";
    return "apoio";
}
...
export function composeLayout(analysis, pieces) {
    ...
    const tallWall = candidates.find((w) => analysis.walls[w].allowsTall && w !== primary) ??
    ...
    const pick = (cls, width, requested) => {
        const prefer = [];
        if (requested && budgets.has(requested))
            prefer.push(requested);
        if (cls === "alto" || cls === "eletro")
            prefer.push(tallWall);
        if (cls === "base" || cls === "aereo")
            prefer.push(baseWall);
        ...
        for (const w of ordered) {
            ...
            if (cls === "alto" && !b.allowsTall)
                continue;
            if (b.used + width > b.capacity)
                continue;
            if (!best) {
                best = b;
                continue;
            ...
            if (b.weight + 0.4 < best.weight)
                best = b;
        }
        if (best)
            return best.wall;
        // 2a passada: aceita a parede com mais folga restante.
        const fallback = [...budgets.values()].sort((a, b) => b.capacity - b.used - (a.capacity - a.used))[0];
    ...
    const sorted = [...enriched].sort((a, b) => CLASS_ORDER[a.cls] - CLASS_ORDER[b.cls] || a.index - b.index);
    const out = [];
    ...
    for (const e of sorted) {
        ...
        if (e.cls === "aereo") {
            ...
            if (e.cls === "base" && !aereoWallByBase.has("base"))
                aereoWallByBase.set("base", wall);
        }
    ...
    notes.push(`Composicao: ${out.length} grupos distribuidos em ${usedWalls.size} parede(s) (${[...usedWalls].join(", ")})`);
    ...
    for (const b of budgets.values())
        analysis.walls[b.wall].load = b.weight / totalWeight;
    return { shape, pieces: out, notes };
}
function promoteShape(base, wallsUsed) {
    if (wallsUsed >= 3)
        return base === "paralela" ? "U" : "U";
    if (wallsUsed === 2)
        return base === "paralela" ? "paralela" : "L";
