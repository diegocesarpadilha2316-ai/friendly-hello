...
import type { PlannerAgentId } from "./types";
...
export function routeByText(message: string): readonly PlannerAgentId[] {
  const t = norm(message);
  const hits: PlannerAgentId[] = [];
...
export function routeByTools(tools: readonly string[]): readonly PlannerAgentId[] {
...
): readonly PlannerAgentId[] {
  const merged = [...routeByTools(tools), ...routeByText(message)];
  const unique = sortAgents(merged);
  return unique.length > 0 ? unique : ["designer"];
}

export function sortAgents(ids: readonly PlannerAgentId[]): PlannerAgentId[] {
  const seen = new Set<PlannerAgentId>();
  const out: PlannerAgentId[] = [];
  for (const id of ids) {
    if (seen.has(id)) continue;
    seen.add(id);
    out.push(id);
  }
  const order = new Map(PLANNER_AGENTS.map((a) => [a.id, a.order] as const));
