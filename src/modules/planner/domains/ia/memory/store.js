...
const SECRET_RE = /(sk-[a-z0-9]|dio_[a-z0-9]|bearer\s|api[_-]?key|secret|token|password|senha\s*[:=]|eyJ[a-z0-9]{6,})/i;
const MAX_ITEMS = 24;
const MAX_VALUE = 160;
export function memoryKey(tenantId, projectId) {
...
export function subscribeMemory(tenantId, projectId, fn) {
...
export function readMemory(tenantId, projectId, projectName) {
    const key = memoryKey(tenantId, projectId);
...
export function clearMemory(tenantId, projectId, projectName) {
    const key = memoryKey(tenantId, projectId);
    if (typeof window !== "undefined") {
        try {
 ...
 */
export function upsertFacts(list, incoming) {
...
export function upsertPendings(list, incoming, resolved = []) {
    const map = new Map(list.map((p) => [p.kind, p]));
    const changed = [];
    for (const kind of resolved) {
        if (map.delete(kind))
            changed.push(`pendencia:${kind}`);
    }
    for (const pending of incoming) {
        if (map.has(pending.kind))
            continue;
...
export function recordMemoryTelemetry(entry) {
...
export function listMemoryTelemetry(projectId) {
