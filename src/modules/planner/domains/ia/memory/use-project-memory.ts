...
import { emptyMemory, type ProjectMemory } from "./types";
...
export function useProjectMemory() {
  ...
  const subscribe = useCallback(
    (fn: () => void) => (projectId ? subscribeMemory(tenantId, projectId, fn) : () => {}),
    [tenantId, projectId],
  );
  ...
  const value: ProjectMemory | null =
    memory ?? (projectId ? emptyMemory(tenantId, projectId, projectName) : null);
