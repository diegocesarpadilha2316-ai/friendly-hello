/**
 * Etapa 10 — serviço de atualização da memória do projeto.
 *
 * Ponto único de escrita: recebe o turno concluído (mensagem do usuário,
 * projeto final e tool calls bem-sucedidas), extrai apenas fatos
 * confirmados, resolve conflitos por chave e regrava a memória.
 */
...
export interface MemoryTurn {
  ...
  /** Turnos cancelados/erro nunca alimentam a memória. */
  readonly outcome: "done" | "error" | "cancelled";
  readonly agent?: string;
}

export function updateMemoryFromTurn(turn: MemoryTurn): ProjectMemory | null {
  if (turn.outcome !== "done") return null;
  ...
  if (style !== current.style) changed.push("estilo");
  const environmentType = extracted.environmentType ?? current.identity.environmentType;
  if (environmentType !== current.identity.environmentType) changed.push("ambiente");
  const stage = extracted.stage ?? current.identity.stage;
  if (stage !== current.identity.stage) changed.push("etapa");
  if (turn.project.name !== current.identity.projectName) changed.push("nome");
  ...
  const withSummary: ProjectMemory = { ...next, executiveSummary: buildExecutiveSummary(next) };

  if (!changed.length && withSummary.executiveSummary === current.executiveSummary) return current;
...
export function recomputeMemory(tenantId: string, projectId: string, projectName?: string) {
