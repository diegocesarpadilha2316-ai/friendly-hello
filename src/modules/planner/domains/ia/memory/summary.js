...
const join = (values) => values.slice(0, MAX_LINE_ITEMS).join("; ");
export function buildExecutiveSummary(memory) {
    const parts = [];
    const env = memory.identity.environmentType;
    parts.push(`${memory.identity.projectName}${env ? ` — ${env}` : ""}`);
    if (memory.style)
        parts.push(`estilo ${memory.style}`);
    const mainMaterial = memory.materials[0]?.value;
    if (mainMaterial)
        parts.push(`acabamento ${mainMaterial}`);
...
export function buildMemoryPromptBlock(memory) {
    ...
    const lines = [];
    if (memory.style)
        lines.push(`Estilo: ${memory.style}.`);
    if (memory.materials.length)
        lines.push(`Materiais aplicados: ${join(memory.materials.map((m) => `${m.key.split(":")[1] ?? m.key}=${m.value}`))}.`);
