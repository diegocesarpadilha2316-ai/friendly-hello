...
export function useProjectMemory() {
    ...
    const subscribe = useCallback((fn) => (projectId ? subscribeMemory(tenantId, projectId, fn) : () => { }), [tenantId, projectId]);
    const getSnapshot = useCallback(() => (projectId ? readMemory(tenantId, projectId, projectName) : null), [tenantId, projectId, projectName]);
    ...
    return {
        ...
        recompute: useCallback(() => {
            if (projectId)
                recomputeMemory(tenantId, projectId, projectName);
        }, [tenantId, projectId, projectName]),
        clear: useCallback(() => {
            if (projectId)
                clearMemory(tenantId, projectId, projectName);
        }, [tenantId, projectId, projectName]),
    };
}
