...
export const MARKETPLACE_REVIEWS: readonly MarketplaceReview[] = MARKETPLACE_ITEMS.flatMap(
  (item, idx) => [
    make(
      `rev-${item.id}-1`,
      item.id,
      "Ana Marcenaria",
      5,
      "Encaixou perfeitamente no Planner Dioris.",
      42 + idx,
      4 + (idx % 20),
    ),
    make(
      ...
      "Estúdio Nex",
      4,
      "Ótima biblioteca, faltou apenas mais variação de puxadores.",
      18 + idx,
      6 + (idx % 15),
    ),
    make(
      ...
      "Marcelo Móveis",
      5,
      "Instalação e atualização automáticas funcionaram lindamente.",
      24 + idx,
      12 + (idx % 10),
    ),
  ],
