...
export function answer(question: string): MarketplaceAIResponse {
  ...
  if (q.includes("fabricante") && q.includes("modul")) {
    ...
    if (top)
      return {
        question,
        answer: `${top[0].toUpperCase()} possui ${top[1]} módulos publicados.`,
        evidenceIds: [],
      };
  }
  if (q.includes("mais usad") || q.includes("mais favoritad")) {
    ...
    if (top)
      return {
        question,
        answer: `O item mais favoritado é ${top.name} (${top.favorites} favoritos).`,
        evidenceIds: [top.id],
      };
  }
  if (q.includes("colecao") || q.includes("coleção") || q.includes("collection")) {
    ...
    if (first)
      return {
        question,
        answer: `Recomendo começar pela coleção "${first.name}" (${first.itemIds.length} itens).`,
        evidenceIds: [...first.itemIds],
      };
  }
  if (q.includes("bem avaliad") || q.includes("melhor avali")) {
    ...
    if (top)
      return {
        question,
        answer: `O item mais bem avaliado é ${top.name} (${top.rating.average.toFixed(1)} ★).`,
        evidenceIds: [top.id],
      };
  }
  const snap = snapshot();
