...
export function searchItems(filters: MarketplaceSearchFilters): readonly MarketplaceItem[] {
  ...
  return MARKETPLACE_ITEMS.filter((item) => {
    ...
    if (
      ...
      (!item.collectionId || !filters.collections.includes(item.collectionId))
    )
      return false;
    ...
    if (typeof filters.minRating === "number" && item.rating.average < filters.minRating)
      return false;
    if (filters.plannerVersion && !isCompatible(item, filters.plannerVersion)) return false;
    if (query) {
      const hay = normalize(
        `${item.name} ${item.description} ${item.brand} ${item.tags.join(" ")}`,
      );
