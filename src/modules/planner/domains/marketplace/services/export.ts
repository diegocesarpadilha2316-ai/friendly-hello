...
export function toCSV(items: readonly MarketplaceItem[]): string {
  const header = ["id", "name", "brand", "category", "version", "license", "price", "downloads", "rating"];
...
export function toPDFText(items: readonly MarketplaceItem[]): string {
  const lines = items.map((i) => `${i.brand.toUpperCase()} • ${i.name} v${i.version} — ${i.pricing.kind === "free" ? "Gratuito" : `R$ ${i.pricing.amount.toFixed(2)}`}`);
  return `Dioris Marketplace\n\n${lines.join("\n")}`;
}

export function exportItems(items: readonly MarketplaceItem[], format: MarketplaceExportFormat): string {
  switch (format) {
    case "csv": return toCSV(items);
    case "json": return toJSON(items);
    case "xml": return toXML(items);
    case "excel": return toExcel(items);
    case "pdf": return toPDFText(items);
  }
}
