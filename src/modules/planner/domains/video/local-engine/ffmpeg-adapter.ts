...
function codecArg(spec: LocalVideoOutputSpec): readonly string[] {
  switch (spec.codec) {
    case "h264": return ["-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "medium"];
    case "h265": return ["-c:v", "libx265", "-pix_fmt", "yuv420p", "-preset", "medium"];
    case "vp9": return ["-c:v", "libvpx-vp9", "-pix_fmt", "yuv420p", "-b:v", `${spec.bitrateKbps}k`];
    case "av1": return ["-c:v", "libaom-av1", "-pix_fmt", "yuv420p"];
    case "prores": return ["-c:v", "prores_ks", "-profile:v", "3"];
...
export function buildFfmpegPlan(job: LocalVideoJob, framesGlob = "frames/%06d.png"): FfmpegPlan {
  ...
  const args = [
    ...
    "-i", framesGlob,
    "-vf", `scale=${spec.resolution.width}:${spec.resolution.height}`,
    ...codecArg(spec),
