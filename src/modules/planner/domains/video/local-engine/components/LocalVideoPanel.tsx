...
type Tab =
  "projeto" | "cameras" | "animacoes" | "timeline" | "audio" | "export" | "fila" | "performance";
...
export function LocalVideoPanel() {
  ...
  const removeClip = (id: string) =>
    setTimeline(
      buildTimeline({
        fps: timeline.fps,
        clips: timeline.clips.filter((c) => c.id !== id),
        tracks: timeline.tracks,
      }),
    );
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_400px]">
      ...
      <section className="flex min-h-[520px] flex-col overflow-hidden rounded-2xl border border-border/60 bg-[radial-gradient(120%_80%_at_50%_-10%,hsl(var(--primary)/0.12),transparent),linear-gradient(180deg,#0a0d17,#04050a)]">
        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-border/50 px-4 py-2">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge tone="info">
              <Film className="mr-1 h-3 w-3" /> Motor Local · Vídeo
            </StatusBadge>
            ...
            <StatusBadge tone="neutral">
              {output.container.toUpperCase()} · {output.codec}
            </StatusBadge>
          ...
          <div className="flex items-center gap-1">
            {(["realtime", "preview", "before-after", "compare", "fullscreen"] as const).map(
              (m) => (
                <button
                  key={m}
                  type="button"
        ...
        <footer className="border-t border-border/50 bg-background/40 px-4 py-2 backdrop-blur">
          <div className="flex items-center justify-between text-[10px] uppercase tracking-widest text-muted-foreground">
            <span>
              Timeline · {totalFrames(timeline)} frames ·{" "}
              {timecode(totalFrames(timeline), timeline.fps)}
            </span>
          ...
          <div className="mt-2 flex h-10 overflow-hidden rounded-lg ring-1 ring-inset ring-border/50">
            {timeline.clips.map((c, i) => {
              ...
              return (
                ...
                >
                  <span className="truncate px-2 opacity-80 group-hover:opacity-0">
                    {move?.label ?? c.moveId}
                  </span>
      ...
      <aside className="rounded-2xl border border-border/60 bg-background/40 backdrop-blur">
        ...
        <div className="max-h-[580px] overflow-auto p-3">
          {tab === "projeto" && (
            <FormSection
              title="Escopo & qualidade"
              description="De onde vem o vídeo e com qual qualidade."
            >
                  ...
                  {LOCAL_CAMERA_MOVES.map((m) => (
                    ...
                    >
                      ...
                      <div className="text-[9px] text-muted-foreground">
                        {m.defaultDurationSec}s · {m.easing}
                      </div>
                    </button>
                  ))}
                </div>
          ...
          {tab === "animacoes" && (
            <FormSection
              title="Animações de cena"
              description="Portas, gavetas, LED, iluminação, estrutura, ferragens."
            >
            ...
            <FormSection title="Timeline" description="FPS, transições, clips.">
              ...
              <div className="mt-3 space-y-1">
                ...
                {timeline.clips.map((c) => (
                  <div
                    key={c.id}
                    className="flex items-center justify-between rounded-md border border-border/40 bg-muted/10 px-2 py-1 text-[11px]"
                  >
                    <span>{c.label}</span>
          ...
          {tab === "audio" && (
            <FormSection
              title="Áudio (algoritmo próprio)"
              description="Preparação para trilha, narração e legenda — sem integrar APIs."
            >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAudio({ ...audio, musicMood: e.target.value as LocalAudioMood })
                    }
                  >
                    {(
                      [
                ...
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAudio({ ...audio, narrationVolume: Number(e.target.value) })
                    }
                  />
                </label>
                <label className="col-span-2 flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    className="accent-primary"
                    checked={audio.subtitleEnabled}
                    onChange={(e) => setAudio({ ...audio, subtitleEnabled: e.target.checked })}
                  />
                  Legenda embutida
                </label>
                <label className="col-span-2 flex items-center gap-2 text-[11px]">
                  <input
                    type="checkbox"
                    className="accent-primary"
                    checked={audio.syncToClips}
                    onChange={(e) => setAudio({ ...audio, syncToClips: e.target.checked })}
                  />
          ...
          {tab === "export" && (
            <FormSection
              title="Exportação"
              description="Container, codec, aspecto, resolução e bitrate."
            >
              <div className="grid grid-cols-2 gap-2">
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setOutput({ ...output, container: e.target.value as LocalVideoContainer })
                    }
                  >
                    {LOCAL_VIDEO_CONTAINERS.map((c) => (
                ...
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <select
                    ...
                    value={output.codec}
                    onChange={(e) =>
                      setOutput({ ...output, codec: e.target.value as LocalVideoCodec })
                    }
                  >
                    {LOCAL_VIDEO_CODECS.map((c) => (
                      <option key={c} value={c}>
                        {c}
                      </option>
                    ))}
                ...
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <select
                    ...
                    value={output.aspect}
                    onChange={(e) =>
                      setOutput({ ...output, aspect: e.target.value as LocalVideoAspect })
                    }
                  >
                    {LOCAL_VIDEO_ASPECTS.map((a) => (
                ...
                <label className="text-[11px] text-muted-foreground">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const r =
                        LOCAL_VIDEO_RESOLUTIONS.find((x) => x.label === e.target.value) ??
                        LOCAL_VIDEO_RESOLUTIONS[1];
                      setOutput({ ...output, resolution: r });
                    }}
                  >
                    {LOCAL_VIDEO_RESOLUTIONS.map((r) => (
                ...
                <label className="col-span-2 flex items-center gap-2 text-[11px]">
                  <input
                    ...
                    onChange={(e) =>
                      setOutput({ ...output, transparentBackground: e.target.checked })
                    }
                  />
          ...
          {tab === "fila" && (
            <div className="space-y-3">
              <FormSection
                ...
                description="Jobs enfileirados/executando."
              >
                {queue.length === 0 && (
                ...
                {queue.map((j) => (
                  ...
                  >
                    ...
                    <div className="mt-1 flex justify-between text-[9px] text-muted-foreground">
                      <span>{j.stage}</span>
                      <span>
                        {j.frameCursor}/{j.frameTotal} frames
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <FormSection
                ...
                description="Últimos jobs concluídos."
              >
                {history.length === 0 && (
                  <p className="text-[11px] text-muted-foreground">Nada por aqui ainda.</p>
                )}
                {history.length > 0 && (
                  <Button variant="outline" className="mb-2" onClick={clearHistory}>
                    Limpar histórico
                  </Button>
                )}
                {history.map((j) => (
          ...
          {tab === "performance" && (
            <FormSection
              title="Performance"
              description="Cache, streaming, render incremental, frame skip, compressão, paralelismo."
            >
              <ul className="grid grid-cols-2 gap-1.5 text-[11px] text-muted-foreground">
                {(playbook
                  ? [
          ...
          <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
            {scene ? (
              <>
                ...
                <span>·</span>
                <span>{scene.openableCount} aberturas</span>
                <span>·</span>
                <span>{scene.lightCount} luzes</span>
                <span>·</span>
