...
export function exportImport(result: ImportResult, format: ImporterExportFormat): ExportBlob {
  ...
  if (format === "dxf")
    return { content: toDXF(result), mime: "application/dxf", filename: `${base}.dxf` };
  if (format === "dwg")
    return { content: toDXF(result), mime: "application/acad", filename: `${base}.dxf` };
  if (format === "obj")
    return { content: toOBJ(result), mime: "text/plain", filename: `${base}.obj` };
  if (format === "glb")
    return {
      content: JSON.stringify({ scenes: [], nodes: [], meshes: [] }, null, 2),
      mime: "model/gltf+json",
      filename: `${base}.gltf`,
    };
  // pdf: stub textual (a fase 3.11/3.13 já possui exportação PDF real por outro caminho).
  return {
    content: `Import ${result.filename} — ${result.entities.length} entidades`,
    mime: "text/plain",
    filename: `${base}.txt`,
  };
}
