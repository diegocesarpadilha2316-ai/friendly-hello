...
export function validateImport(res: ImportResult): readonly ImporterWarning[] {
  ...
  if (res.entities.length === 0 && !res.previewSvg) {
    out.push({ code: "empty", message: "Arquivo importado não contém entidades reconhecidas.", severity: "warning" });
  }
  if (!res.bbox) {
    out.push({ code: "no-bbox", message: "Sem bounding box detectado — escala pode ficar imprecisa.", severity: "info" });
  }
