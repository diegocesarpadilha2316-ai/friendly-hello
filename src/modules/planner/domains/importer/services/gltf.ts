...
interface GLTFDoc {
  ...
  readonly materials?: { name?: string }[];
}
...
export function parseGLTF(text: string, filename: string): ImportResult {
  ...
  const mats: ImporterMaterialRef[] = Array.isArray(doc?.materials)
    ? (doc!.materials as { name?: string }[]).map((m, i) => ({
        id: m.name ?? `mat-${i}`,
  ...
  return {
    ...
    bbox: null,
    layers: [
      { id: "GLTF", name: "Scene", visible: true, locked: false, count: nodes, role: "furniture" },
    ],
    entities: [],
    materials: mats,
    texts: [],
    ...
    warnings: [
      {
        code: "gltf-scene",
        severity: "info",
        message: `glTF com ${nodes} nós / ${meshes} meshes.`,
      },
    ],
