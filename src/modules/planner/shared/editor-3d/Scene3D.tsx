...
function Furniture({
  ...
  if (f.subtype === "volume-tecnico" || (f.params as any)?.role === "validation" || (f.params as any)?.role === "auxiliary") return null;
  ...
  const laundry = viewport.render !== "wireframe" && decision.renderer === "laundry";
  if (laundry) {
    return (
      <group
        name={`furniture-${f.id}`}
