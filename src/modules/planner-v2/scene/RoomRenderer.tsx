...
const WallMesh: React.FC<WallMeshProps & { visible?: boolean }> = ({
  wall,
  mode,
  visible = true,
}) => {
...
const DoorMesh: React.FC<{
  wall: WallGeometry;
  opening: { x: number; width: number; height: number };
}> = ({ wall, opening }) => {
...
export const RoomRenderer: React.FC<RoomRendererProps> = ({
  ...
  const floorMaterial = useMemo(() => {
    if (mode === "technical")
      return new THREE.MeshStandardMaterial({ color: "#d1c7bc", roughness: 0.7 });
  ...
  const visibleWallIds = useMemo(() => {
    ...
    const center = new THREE.Vector3(
      result.floor.width / 2,
      result.bounds.max[1] / 2,
      result.floor.depth / 2,
    );
  ...
  return (
    <group>
      ...
      <mesh position={[centerX, -result.floor.thickness / 2, centerZ]} receiveShadow>
