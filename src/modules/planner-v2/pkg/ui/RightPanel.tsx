...
export function RightPanel() {
  ...
  const [materialSlot, setMaterialSlot] = useState<keyof FurnitureInstance["materialOverrides"] | "body">("body");
  ...
  const downloadCutList = () => {
    const blob = new Blob([fabricationReportToCsv(fabricationReport)], { type: "text/csv;charset=utf-8" });
  ...
  const availableMaterials = activeInstance
    ...
    : materials.map(([label, id, color]) => ({ name: label, id, baseColor: color, category: "mdf" as const }));
  ...
  return (
    <aside className={`right-panel ${collapsed ? "collapsed" : ""}`}>
      <div className="panel-head">
        ...
        <button type="button" onClick={toggleRight} aria-label={collapsed ? "Expandir painel direito" : "Recolher painel direito"} title={collapsed ? "Expandir painel direito" : "Recolher painel direito"}>
          ...
          <div className="right-content">
            {rightTab === "chat" && (
              <div className="chat">
                ...
                <div className="quick-actions">
                  <button type="button" onClick={closeAll}>Fechar Tudo</button>
                  <button type="button" onClick={showAll}>Mostrar Tudo</button>
                  <button type="button" onClick={() => sendMessage("Trocar material")}>Trocar material</button>
                  <button type="button" onClick={() => sendMessage("Ver orçamento")}>Ver orçamento</button>
                </div>
                ...
                {activeInstance ? (
                  <>
                    <div className="form-actions">
                      ...
                      <button type="button" onClick={() => deleteInstance(activeInstance.id)}>Excluir</button>
                    </div>
                    <div className="form-actions">
                      <button type="button" onClick={() => isolate(activeInstance.isIsolated ? null : activeInstance.id)}>
                      ...
                      <button type="button" onClick={() => duplicateInstance(activeInstance.id)}>Duplicar</button>
                      <button type="button" onClick={() => activeInstance.locked ? unlockInstance(activeInstance.id) : lockInstance(activeInstance.id)}>{activeInstance.locked ? "Desbloquear" : "Bloquear"}</button>
                    </div>
                    <div className="transform-modes" role="toolbar" aria-label="Modos de transformação">
                      {([['move', 'Mover'], ['rotate', 'Rotacionar'], ['dimensions', 'Medidas']] as const).map(([mode, label]) => (
                        <button key={mode} type="button" className={toolMode === mode ? "active" : ""} onClick={() => setToolMode(mode)}>{label}</button>
                      ))}
                    </div>
                    {toolMode === "move" && (
                      <div className="form-actions transform-controls">
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { positionMm: { ...activeInstance.positionMm, x: activeInstance.positionMm.x - 50 } })}>X −</button>
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { positionMm: { ...activeInstance.positionMm, x: activeInstance.positionMm.x + 50 } })}>X +</button>
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { positionMm: { ...activeInstance.positionMm, z: activeInstance.positionMm.z - 50 } })}>Z −</button>
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { positionMm: { ...activeInstance.positionMm, z: activeInstance.positionMm.z + 50 } })}>Z +</button>
                      </div>
                    )}
                    {toolMode === "rotate" && (
                      <div className="form-actions transform-controls">
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { rotationDeg: { ...activeInstance.rotationDeg, y: activeInstance.rotationDeg.y - 15 } })}>−15°</button>
                        <button type="button" onClick={() => updateInstance(activeInstance.id, { rotationDeg: { ...activeInstance.rotationDeg, y: activeInstance.rotationDeg.y + 15 } })}>+15°</button>
                      </div>
                    )}
                    <div className="form-actions">
                      <button type="button" onClick={() => snapInstance(activeInstance.id)}>Aplicar Snap</button>
                      <button type="button" onClick={() => setSnapEnabled(!snapEnabled)}>{snapEnabled ? "Snap ligado" : "Snap desligado"}</button>
                    </div>
                    {lastSnapMessage && <small className="panel-note">{lastSnapMessage}</small>}
                    {(["width", "height", "depth"] as const).map((axis) => (
                      <label key={axis}>
                        {axis === "width" ? "Largura" : axis === "height" ? "Altura" : "Profundidade"} (mm)
                        <input
                          type="number"
                          step={10}
                    ...
                    {(["x", "y", "z"] as const).map((axis) => (
                      <label key={axis}>Posição {axis.toUpperCase()} (mm)
                        <input
                          type="number"
                          step={50}
                    ...
                    <label>Rotação Y (graus)
                      <input
                        ...
                        onChange={(event) => updateInstance(activeInstance.id, {
                          rotationDeg: { ...activeInstance.rotationDeg, y: Number(event.target.value) }
                        })}
                      />
                    </label>
                  </>
                  ...
                  <>
                    <label>Largura (m)<input type="number" step={0.05} value={selected?.size[0] ?? 0} onChange={(event) => selected && updateSelected({ size: [Number(event.target.value), selected.size[1], selected.size[2]] })} /></label>
                    <label>Altura (m)<input type="number" step={0.05} value={selected?.size[1] ?? 0} onChange={(event) => selected && updateSelected({ size: [selected.size[0], Number(event.target.value), selected.size[2]] })} /></label>
                    <label>Profundidade (m)<input type="number" step={0.05} value={selected?.size[2] ?? 0} onChange={(event) => selected && updateSelected({ size: [selected.size[0], selected.size[1], Number(event.target.value)] })} /></label>
                  </>
                )}
              </div>
            )}
            ...
            {rightTab === "materials" && (
              <div className="swatches">
                {activeInstance && (
                  <label className="material-slot">Aplicar em
                    <select value={materialSlot} onChange={(event) => setMaterialSlot(event.target.value as typeof materialSlot)}>
                      {materialSlots.map(([label, slot]) => <option key={slot} value={slot}>{label}</option>)}
                    </select>
                  </label>
                )}
                {availableMaterials.map((item) => (
                  ...
                  onClick={() => {
                    if (activeInstance) updateInstance(activeInstance.id, { materialOverrides: { ...activeInstance.materialOverrides, [materialSlot]: item.id } });
                ...
                {hardwareOptions.map(([label, role, fallback, category]) => (
                  <label key={role}>{label}
                    <select
                      ...
                      onChange={(event) => activeInstance && updateInstance(activeInstance.id, {
                        hardwareOverrides: { ...activeInstance.hardwareOverrides, [role]: event.target.value }
                      })}
                    >
                      <option value={fallback}>{label} padrão</option>
                      {HardwareRegistry.listByCategory(category as "handle" | "slide" | "hinge").map((hardware) => <option key={hardware.id} value={hardware.id}>{hardware.name}</option>)}
                    </select>
                  </label>
                ))}
              </div>
            ...
            {rightTab === "fabrication" && (
              <div className="form fabrication-panel">
                <div className="form-actions">
                  ...
                  <span>{fabricationReport.cutItems.reduce((sum, item) => sum + item.quantity, 0)} peças físicas</span>
                  <button type="button" onClick={downloadCutList}>Baixar CSV</button>
                </div>
                {fabricationReport.warnings.length > 0 && <div className="panel-note">Avisos: {fabricationReport.warnings.join(" ")}</div>}
                ...
                <div className="fabrication-list">
                  {fabricationReport.cutItems.map((item) => (
                    <div key={item.key} className="fabrication-row">
                      <strong>{item.quantity}× {item.names.join(" / ")}</strong>
                      <small>{item.widthMm} × {item.heightMm} × {item.depthMm} mm · {item.materialId}</small>
                    </div>
                  ))}
                </div>
                <h4>Nesting MaxRects</h4>
                <div className="form-actions"><span>{nestingPlan.statistics.boardsCount} chapas</span><span>{Math.round(nestingPlan.statistics.avgUsageRatio * 100)}% aproveitamento</span></div>
                {nestingPlan.unplaced.length > 0 && <div className="panel-note">Peças não colocadas: {nestingPlan.unplaced.length}</div>}
                <h4>Usinagem</h4>
                <div className="form-actions"><span>{joineryReport.operations.length} operações</span><span>{joineryReport.warnings.length} avisos</span></div>
                <h4>Ferragens</h4>
                {fabricationReport.hardwareItems.map((item) => <div key={item.hardwareId} className="fabrication-row"><strong>{item.quantity}× {item.hardwareId}</strong><small>{item.moduleIds.join(", ")}</small></div>)}
                <h4>Montagem</h4>
                <div className="fabrication-list">
                  {assemblyReport.steps.map((step) => <div key={step.id} className="fabrication-row"><strong>{step.order}. {step.title}</strong><small>{step.instruction}</small></div>)}
                </div>
              </div>
            )}
          </div>
