...
export function RenderFinalPanel({ onClose }: { onClose: () => void }) {
  ...
  const render = (view: RenderView) => {
    ...
    window.dispatchEvent(
      new CustomEvent("dioris:render-image", { detail: { view, quality, ...resolution } }),
    );
    window.setTimeout(() => {
      setBusy(false);
      setStatus(`Solicitação ${view} enviada ao pipeline PNG real.`);
    }, 600);
  };

  const renderAll = () =>
    views.forEach((view, index) => window.setTimeout(() => render(view.id), index * 900));
  ...
  return (
    <aside className="render-final-panel" role="dialog" aria-label="Render Final">
      <div className="render-final-header">
        <div>
          <strong>Render Final</strong>
          <span>Pipeline separado do viewport de edição</span>
        </div>
        <button
          type="button"
          className="render-close"
          onClick={onClose}
          aria-label="Fechar Render Final"
        >
          ×
        </button>
      </div>
      <p className="render-final-copy">
        A captura usa a cena real sem alterar FurnitureInstance, seleção, abertura ou persistência.
      </p>
      <label className="render-quality-label" htmlFor="render-quality">
      ...
      <div className="render-view-grid">
        {views.map((view) => (
          <button type="button" key={view.id} onClick={() => render(view.id)} disabled={busy}>
            {view.label}
          </button>
        ))}
      </div>
      <button type="button" className="render-all-button" onClick={renderAll} disabled={busy}>
        Gerar as vistas V10 de evidência
      </button>
      <div className="render-video-section">
      ...
      <div className={`render-status ${busy ? "is-busy" : ""}`} role="status" aria-live="polite">
        {status}
      </div>
      <small className="render-format-note">
        PNG fiel ao projeto paramétrico. Vídeo WebM via MediaRecorder quando suportado pelo
        navegador.
      </small>
    </aside>
  );
