  ...
  return (
    <aside className={`explorer ${collapsed ? "collapsed" : ""}`}>
      <div className="panel-head">
        ...
        <button type="button" onClick={toggleLeft} aria-label={collapsed ? "Expandir explorador" : "Recolher explorador"} title={collapsed ? "Expandir explorador" : "Recolher explorador"}>
      ...
      {!collapsed && (
        <div className="panel-tabs">
          ...
          <button className={activeTab === "reference" ? "active" : ""} onClick={() => setActiveTab("reference")}>
            Imagem IA
          </button>
        </div>
      )}
          ...
          {instances.length > 0 ? instances.map((instance) => (
            <div key={instance.id}>
              <button
                ...
                onClick={() => { selectFurnitureInstance(instance.id); setExpandedInstances((current) => ({ ...current, [instance.id]: !current[instance.id] })); }}
                title={collapsed ? instance.name : undefined}
              >
                {expandedInstances[instance.id] ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                <Box size={15} />
                {!collapsed && <span>{instance.name}</span>}
                {!collapsed && <span className="grow" />}
                {!collapsed && (
                  <>
                    <span className="inline-action" onClick={(event) => { event.stopPropagation(); instance.visible ? hideFurnitureInstance(instance.id) : showFurnitureInstance(instance.id); }}>
              ...
              {!collapsed && expandedInstances[instance.id] && instance.parts.map((part) => (
                <button type="button" className="tree-row child part-row" key={part.id} onClick={() => selectFurnitureInstance(instance.id)} title={part.name}>
