...
const NATURAL_KITCHEN_REQUEST = "Crie uma cozinha nessa parede. Quero uma torre de forno e micro-ondas na esquerda, um balcão de 800 com duas portas, um gaveteiro de 600 com quatro gavetas, um balcão de pia de 1200 com duas portas e aéreos em cima. Use MDF 18 mm.";
...
describe("manufacturing integration — natural request to fabrication", () => {
  it("materializa, abre e documenta a cozinha sem posicionamento manual", () => {
    ...
    const sink = instances.find((instance) => instance.moduleDefinitionId === "kitchen-sink-cabinet");
    ...
    writeFileSync(resolve(evidenceDir, "natural-kitchen-manufacturing.json"), JSON.stringify({
      request: NATURAL_KITCHEN_REQUEST,
      modules: instances.map((instance) => ({ id: instance.id, moduleDefinitionId: instance.moduleDefinitionId, dimensionsMm: instance.dimensionsMm, positionMm: instance.positionMm, supported: instance.layout?.supported !== false, partCount: instance.parts.length })),
      fabrication,
      joinery,
      nesting,
    }, null, 2));
    writeFileSync(resolve(evidenceDir, "natural-kitchen-cut-list.csv"), fabricationReportToCsv(fabrication));
    const boardSvg = nesting.boards.map((board) => {
      const scale = 0.18;
      const rects = board.placements.map((placement) => `<rect x="${placement.x * scale}" y="${placement.y * scale}" width="${placement.w * scale}" height="${placement.h * scale}" fill="#b7c9a8" stroke="#203225"/><text x="${placement.x * scale + 3}" y="${placement.y * scale + 12}" font-size="10">${placement.code.slice(-12)}</text>`).join("");
    ...
    writeFileSync(resolve(evidenceDir, "natural-kitchen-nesting.svg"), `<svg xmlns="http://www.w3.org/2000/svg" width="520" height="${Math.max(380, nesting.boards.length * 360)}">${boardSvg}</svg>`);
  });
});
