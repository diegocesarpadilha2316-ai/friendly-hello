...
import {
  moduleIdForNaturalRequest,
  parseDesignIntent,
  parseKitchenComposition,
...
describe("CompositionSpec natural de cozinha", () => {
  it("decompõe uma cozinha em ModuleSpec oficiais com espessura de MDF", () => {
    const spec = parseKitchenComposition(
      "Crie uma cozinha com torre 700 mm, balcão 800 mm, gaveteiro 600 mm, pia 1200 mm e aéreos em cima, tudo em MDF 18 mm.",
    );
    ...
    expect(
      spec?.modules.find((module) => module.moduleId === "kitchen-sink-cabinet")?.dimensionsMm
        .width,
    ).toBe(1200);
  });
  ...
  it("mantém espessuras alternativas por pedido natural", () => {
    const spec = parseKitchenComposition(
      "Monte uma cozinha com balcão 800 mm, gaveteiro 600 mm e pia 800 mm em MDF 25 mm.",
    );
...
it("roteia pedidos incrementais para as famílias oficiais", () => {
  expect(
    moduleIdForNaturalRequest("crie um aéreo de 800 mm com duas portas e três prateleiras"),
  ).toBe("kitchen-golden-upper-800");
  expect(moduleIdForNaturalRequest("adicione ao lado um aéreo de 600 com uma porta")).toBe(
    "kitchen-upper-1-door",
  );
  expect(moduleIdForNaturalRequest("embaixo faça um balcão de pia de 1200 com duas portas")).toBe(
    "kitchen-sink-cabinet",
  );
  expect(
    moduleIdForNaturalRequest("do lado direito quero um gaveteiro de 600 com quatro gavetas"),
  ).toBe("kitchen-drawer-4");
  expect(moduleIdForNaturalRequest("coloque uma torre para forno e micro-ondas no final")).toBe(
    "kitchen-tower-oven-microwave",
  );
});
...
it("produz DesignIntent com fabricação e sem coordenadas manuais", () => {
  const intent = parseDesignIntent(
    "Crie uma cozinha com balcão 800 mm e gaveteiro 600 mm em MDF Freijó 18 mm, com puxador gola e bancada de granito. Gere a lista de corte, BOM e render.",
  );
