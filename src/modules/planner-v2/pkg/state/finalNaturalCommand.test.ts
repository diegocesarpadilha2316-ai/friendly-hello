...
const FINAL_REQUEST =
  "Crie uma cozinha linear com torre de forno, balcão de duas portas, gaveteiro de quatro gavetas, balcão de pia, módulos aéreos e bancada. Use MDF 18mm.";
...
describe("final natural command execution", () => {
  it("gera o projeto completo e exporta todos os dados de fabricação", () => {
    ...
    // 1. BOM & Cutlist
    writeFileSync(
      resolve(evidenceDir, "cozinha-lista-corte.csv"),
    ...
    const boardSvg = nesting.boards
      .map((board) => {
        ...
        const rects = board.placements
          ...
          .join("");
        return (
          `<g transform="translate(0, ${board.index * 380})">` +
    ...
    writeFileSync(
      resolve(evidenceDir, "cozinha-nesting.svg"),
    ...
    writeFileSync(
      resolve(evidenceDir, "cozinha-resumo-tecnico.json"),
