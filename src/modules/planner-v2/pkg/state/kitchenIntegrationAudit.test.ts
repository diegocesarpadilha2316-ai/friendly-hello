...
describe("Auditoria da cozinha de integração", () => {
  it("usa o pedido natural final e posiciona todos os módulos pela sequência da parede", () => {
    ...
    expect(layout.placements.every((placement) => placement.supported && !placement.collision)).toBe(true);
    expect(layout.countertops).toHaveLength(1);
    expect(layout.countertops[0].supportModuleIds).toEqual(["natural-base", "natural-drawer", "natural-sink"]);
    expect(layout.applianceZones.some((zone) => zone.moduleId === "natural-tower")).toBe(true);

    const built: Array<{ id: string; name: string; dimensionsMm: { width: number; height: number; depth: number }; positionMm: { x: number; y: number; z: number } }> = [];
    for (const moduleSpec of spec?.modules ?? []) {
      ...
      expect(outcome.validation?.valid, outcome.validation?.errors.map((error) => error.message).join("; ")).toBe(true);
      ...
      built.push({ id: moduleSpec.id, name: moduleSpec.moduleId, dimensionsMm: outcome.dimensionsMm, positionMm: placement!.positionMm });
    }
