...
describe("Golden hardware manufacturing specification — Etapa 4.1", () => {
  ...
  it("não confunde parafuso com pré-furo e mantém assembly READY separado de machining", () => {
    ...
    expect(hingeFixing).toHaveLength(4);
    expect(hingeFixing.every((operation) => operation.diameterMm === undefined && operation.parameters.pilotHoleDiameterMm === null)).toBe(true);
    expect(hingeFixing.every((operation) => operation.missingParameters.includes("pilotHoleDiameterMm"))).toBe(true);
    expect(platePilot).toHaveLength(4);
    expect(platePilot.every((operation) => operation.diameterMm === undefined)).toBe(true);
    expect(platePilot.every((operation) => operation.parameters.screwDiameterMm === 3.5)).toBe(true);
    expect(platePilot.every((operation) => operation.parameters.pilotHoleDiameterMm === null)).toBe(true);
    expect(platePilot.every((operation) => operation.readiness === "INCOMPLETE")).toBe(true);
    expect(machining.assemblyReadiness.filter((item) => item.hardwareId === "mounting-plate-37-32").length).toBe(8);
    expect(machining.assemblyReadiness.filter((item) => item.hardwareId === "mounting-plate-37-32").every((item) => item.status === "READY")).toBe(true);
  ...
  it("preserva IDs, relações, persistência e determinismo em movimento, rotação e 900→1000→900", () => {
    ...
    expect(restored.machining.operations.map((operation) => operation.coordinates)).toEqual(beforeCoordinates);
    expect(restored.machining.assemblyReadiness.every((item) => item.status === "READY")).toBe(true);
