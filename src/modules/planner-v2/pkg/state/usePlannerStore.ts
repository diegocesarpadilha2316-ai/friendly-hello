...
function extractDimensionTuple(text: string) {
  const match = text.match(/(\d+(?:[.,]\d+)?)\s*[x×]\s*(\d+(?:[.,]\d+)?)(?:\s*[x×]\s*(\d+(?:[.,]\d+)?))?\s*(mm|cm|m)?/i);
...
export function moduleIdForNaturalRequest(text: string) {
  const isUpper = text.includes("aéreo") || text.includes("aereo") || text.includes("aéreos") || text.includes("aereos");
  if (isUpper) {
    ...
    if (text.includes("3 prateleiras") || text.includes("três prateleiras")) return "kitchen-golden-upper-800";
    return "kitchen-upper-2-doors";
  }
  if (text.includes("pia") || text.includes("cuba") || text.includes("sink")) return "kitchen-sink-cabinet";
  ...
  if (text.includes("torre") || text.includes("forno") || text.includes("micro-ondas")) return "kitchen-tower-oven-microwave";
  if (text.includes("ilha")) return "kitchen-island-base";
  if (text.includes("balcão") || text.includes("balcao") || text.includes("base")) return "kitchen-base-2-doors";
  return null;
}
...
export function parseKitchenComposition(text: string): KitchenCompositionSpec | null {
  ...
  const bodyMaterial = /freij[oó]/i.test(text) ? "mdf-freijo" : /grafite/i.test(text) ? "mdf-graphite" : /verde/i.test(text) ? "mdf-green" : /amadeirado|madeira/i.test(text) ? "mdf-wood-natural" : "mdf-white";
  const countertopMaterial = /granito|preto/i.test(text) ? "stone-granite" : /m[aá]rmore/i.test(text) ? "stone-marble" : /quartzo/i.test(text) ? "stone-quartz" : "stone-light";
  const handle = /gola/i.test(text) ? "handle-gola" : /cava/i.test(text) ? "handle-cava" : /perfil/i.test(text) ? "handle-profile" : "handle-bar";
  ...
  const relation = (anchor: "floor" | "wall" | "appliance-zone", sequenceIndex: number, anchorModuleId?: string) => ({ wallId, anchor, sequenceIndex, anchorModuleId, alignment: "front" as const, clearanceMm: 0 });
  const lowerOnlyMarker = /(etapa\s*1|m[oó]dulos? inferiores?|somente a engenharia dos m[oó]dulos|sem a[eé]reos?)/i.test(text);
  const explicitExclusions = /(?:n[aã]o\s+(?:inclua|adicione|quero)|sem)\s+(?:a[eé]reos?|torre|geladeira|frigor[ií]fico|coifa|depurador|cooktop|fog[aã]o|decora[cç][aã]o|planta)/i.test(text);
  const lowerOnly = lowerOnlyMarker && (explicitExclusions || !/(?:inclua|adicione|quero)\s+(?:a[eé]reos?|torre|geladeira|frigor[ií]fico|coifa|depurador|cooktop|fog[aã]o)/i.test(text));
  if (lowerOnly) {
    const baseWidths = [...text.matchAll(/balc(?:a|ã)o(?!\s+de\s+pia)[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/gi)].map((match) => measurementToMm(match[1], match[2]) ?? 800);
    const baseWidthLeft = baseWidths[0] ?? 800;
    const baseWidthRight = baseWidths[1] ?? baseWidthLeft;
    modules.push({ id: "etapa1-base-left", moduleId: "kitchen-base-2-doors", kind: "base", dimensionsMm: { width: baseWidthLeft, height: 870, depth: 580 }, relation: relation("floor", 0) });
    modules.push({ id: "etapa1-drawer", moduleId: "kitchen-drawer-4", kind: "drawer", dimensionsMm: { width: widthFrom(/gaveteiro[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 600), height: 870, depth: 580 }, relation: relation("floor", 1) });
    modules.push({ id: "etapa1-sink", moduleId: "kitchen-sink-cabinet", kind: "sink", dimensionsMm: { width: widthFrom(/pia[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 1200), height: 870, depth: 580 }, relation: relation("floor", 2) });
    modules.push({ id: "etapa1-base-right", moduleId: "kitchen-base-2-doors", kind: "base", dimensionsMm: { width: baseWidthRight, height: 870, depth: 580 }, relation: relation("floor", 3) });
    return { thicknessMm, modules, materials, hardware };
  }
  if (/(torre|forno|micro-ondas)/i.test(text)) modules.push({ id: "natural-tower", moduleId: "kitchen-tower-oven-microwave", kind: "tower", dimensionsMm: { width: widthFrom(/torre[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 700), height: 2200, depth: 620 }, relation: relation("appliance-zone", 0) });
  if (/(balcão|balcao).*(\d+)/i.test(text)) modules.push({ id: "natural-base", moduleId: "kitchen-base-2-doors", kind: "base", dimensionsMm: { width: widthFrom(/balc(?:ão|ao)[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 800), height: 870, depth: 580 }, relation: relation("floor", 1) });
  if (/(gaveteiro|gavetas?)/i.test(text)) modules.push({ id: "natural-drawer", moduleId: /(4\s*gavetas?|quatro\s+gavetas?)/i.test(text) ? "kitchen-drawer-4" : /(2\s*gavetas?|duas\s+gavetas?)/i.test(text) ? "kitchen-drawer-2" : /(1\s*gaveta|uma\s+gaveta)/i.test(text) ? "kitchen-drawer-1" : "kitchen-drawer-3", kind: "drawer", dimensionsMm: { width: widthFrom(/gaveteiro[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 600), height: 870, depth: 580 }, relation: relation("floor", 2) });
  if (/(pia|cuba)/i.test(text)) modules.push({ id: "natural-sink", moduleId: "kitchen-sink-cabinet", kind: "sink", dimensionsMm: { width: widthFrom(/pia[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 1200), height: 870, depth: 580 }, relation: relation("floor", 3) });
  if (/(cooktop|fogão)/i.test(text)) modules.push({ id: "natural-cooktop", moduleId: "kitchen-cooktop-cabinet", kind: "cooktop", dimensionsMm: { width: widthFrom(/cooktop[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 800), height: 870, depth: 580 }, relation: relation("floor", 4) });
  if (/(coifa|depurador)/i.test(text)) modules.push({ id: "natural-hood", moduleId: "kitchen-upper-hood", kind: "hood", dimensionsMm: { width: 800, height: 450, depth: 350 }, relation: relation("wall", 5, "natural-cooktop") });
  if (/(geladeira|frigorífico|fridge)/i.test(text)) modules.push({ id: "natural-fridge", moduleId: "kitchen-tower-fridge", kind: "tower", dimensionsMm: { width: widthFrom(/geladeira[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?/i, 900), height: 2200, depth: 700 }, relation: relation("appliance-zone", 6) });
  if (/(aéreos?|aereos?)/i.test(text)) modules.push({ id: "natural-upper", moduleId: "kitchen-golden-upper-800", kind: "upper", dimensionsMm: { width: 800, height: 700, depth: 350 }, relation: relation("wall", 7, "natural-base") });
...
export function parseDesignIntent(text: string): DesignIntent | null {
  ...
  return {
    ...
    modules: composition.modules.map((module) => ({
      ...
      anchor: (module.relation.anchor === "countertop" ? "floor" : module.relation.anchor) as DesignIntent["modules"][number]["anchor"],
    })),
    constraints: {
      noManualPositioning: true,
      requireLayoutEngine: true,
      requireFabricationReport: requestedOutputs.includes("cut-list") || requestedOutputs.includes("bom") || /marcenaria|fabric[aá]vel|fabrica[cç][aã]o/i.test(normalized),
    },
    requestedOutputs,
  };
}
...
interface PlannerState {
  ...
  addFurnitureInstance: (moduleId: string, positionMm?: { x: number; y: number; z: number }, dimensionsMm?: { width: number; height: number; depth: number }, thicknessMm?: ThicknessProfileMm, layout?: LayoutPlacement) => string | null;
...
type PlannerHistorySnapshot = Pick<PlannerState, "furniture" | "instances" | "selectedId" | "gridVisible" | "lightsEnabled" | "snapEnabled">;
...
export const usePlannerStore = create<PlannerState>()(
  ...
  saveProject: () => {
    ...
    saveProjectToStorage(serializeProject({
      planner: {
    ...
    if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent("dioris:project-saved"));
    return true;
  },
  ...
  loadProject: () => {
    ...
    for (const saved of project.planner.instances) {
      ...
      if (!outcome.ok) {
        set({ lastLibraryError: `Projeto não restaurado: ${saved.name} — ${outcome.error ?? "posição inválida"}` });
        return false;
      }
      restored.push({
        ...saved,
  ...
  undo: () => {
    ...
    set((state) => ({ ...state, ...previous, historyVersion: state.historyVersion + 1, lastLibraryError: null }));
  ...
  redo: () => {
    ...
    historyApplying = true;
    set((state) => ({ ...state, ...next, historyVersion: state.historyVersion + 1, lastLibraryError: null }));
    historyApplying = false;
    get().saveProject();
  },
  ...
  applyGoldenKitchen: () => {
    ...
    const modules: PresetModuleEntry[] = [
      { moduleId: "kitchen-tower-oven-microwave", positionMm: { x: -1900, y: 0, z: -1400 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-profile", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-base-2-doors", positionMm: { x: -1120, y: 0, z: -1400 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-profile" } },
      { moduleId: "kitchen-drawer-3", positionMm: { x: -300, y: 0, z: -1400 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-cava", slide: "slide-hidden-soft-close" } },
      { moduleId: "kitchen-sink-cabinet", positionMm: { x: 520, y: 0, z: -1400 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-cooktop-cabinet", positionMm: { x: 1320, y: 0, z: -1400 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-profile", slide: "slide-hidden-soft-close" } },
      { moduleId: "kitchen-base-2-doors", positionMm: { x: 1760, y: 0, z: -900 }, rotationDeg: { x: 0, y: 90, z: 0 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-profile", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-drawer-3", positionMm: { x: 1760, y: 0, z: -220 }, rotationDeg: { x: 0, y: 90, z: 0 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-cava", slide: "slide-hidden-soft-close" } },
      { moduleId: "kitchen-countertop", positionMm: { x: 100, y: 870, z: -1400 }, dimensionsMm: { width: 3240, height: 20, depth: 600 }, materialId: "stone-quartzite" },
      { moduleId: "kitchen-countertop", positionMm: { x: 1760, y: 870, z: -900 }, rotationDeg: { x: 0, y: 90, z: 0 }, dimensionsMm: { width: 800, height: 20, depth: 600 }, materialId: "stone-quartzite" },
      { moduleId: "kitchen-countertop", positionMm: { x: 1760, y: 870, z: -220 }, rotationDeg: { x: 0, y: 90, z: 0 }, dimensionsMm: { width: 600, height: 20, depth: 600 }, materialId: "stone-quartzite" },
      { moduleId: "kitchen-upper-2-doors", positionMm: { x: -1150, y: 1500, z: -1600 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-upper-glass-2-doors", positionMm: { x: -300, y: 1500, z: -1600 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-none" } },
      { moduleId: "kitchen-upper-2-doors", positionMm: { x: 650, y: 1500, z: -1600 }, materialId: "mdf-freijo", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
    ];
    const createdIds: string[] = [];
    for (const entry of modules) {
      const id = get().addFurnitureInstance(entry.moduleId, entry.positionMm, "dimensionsMm" in entry ? entry.dimensionsMm : undefined);
      if (!id) continue;
      createdIds.push(id);
    ...
    historyFuture.length = 0;
    get().saveProject();
    if (createdIds.length === 0) set((state) => ({ ...state, lastLibraryError: "Não foi possível montar a Golden Kitchen no espaço atual." }));
  },
  ...
  applyPromobReference: () => {
    ...
    useRoomBuilderStore.getState().applyReferencePreset();
    const modules: PresetModuleEntry[] = [
      { moduleId: "kitchen-tower-oven-microwave", positionMm: { x: -1900, y: 0, z: -1400 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-profile", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-base-2-doors", positionMm: { x: -1250, y: 0, z: -1400 }, dimensionsMm: { width: 700, height: 700, depth: 580 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-profile" } },
      { moduleId: "kitchen-drawer-4", positionMm: { x: -625, y: 0, z: -1400 }, dimensionsMm: { width: 500, height: 700, depth: 600 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-cava", slide: "slide-hidden-soft-close" } },
      { moduleId: "kitchen-sink-cabinet", positionMm: { x: 100, y: 0, z: -1400 }, dimensionsMm: { width: 800, height: 700, depth: 580 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-corner-base", positionMm: { x: 850, y: 0, z: -1050 }, dimensionsMm: { width: 1000, height: 700, depth: 834 }, rotationDeg: { x: 0, y: 90, z: 0 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-base-2-doors", positionMm: { x: 1350, y: 0, z: -250 }, dimensionsMm: { width: 800, height: 700, depth: 600 }, rotationDeg: { x: 0, y: 90, z: 0 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-profile" } },
      { moduleId: "kitchen-drawer-3", positionMm: { x: 1350, y: 0, z: 450 }, dimensionsMm: { width: 500, height: 700, depth: 600 }, rotationDeg: { x: 0, y: 90, z: 0 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-cava", slide: "slide-hidden-soft-close" } },
      { moduleId: "kitchen-countertop", positionMm: { x: -500, y: 710, z: -1400 }, dimensionsMm: { width: 2750, height: 20, depth: 600 }, materialId: "stone-granite" },
      { moduleId: "kitchen-countertop", positionMm: { x: 1350, y: 710, z: -650 }, rotationDeg: { x: 0, y: 90, z: 0 }, dimensionsMm: { width: 1900, height: 20, depth: 834 }, materialId: "stone-granite" },
      { moduleId: "kitchen-upper-2-doors", positionMm: { x: -900, y: 1500, z: -1650 }, dimensionsMm: { width: 970, height: 700, depth: 400 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
      { moduleId: "kitchen-upper-corner", positionMm: { x: 700, y: 1500, z: -1650 }, dimensionsMm: { width: 960, height: 700, depth: 760 }, materialId: "mdf-graphite", hardwareOverrides: { handle: "handle-gola", hinge: "hinge-soft-close" } },
    ];
    const createdIds: string[] = [];
    for (const entry of modules) {
    ...
    historyFuture.length = 0;
    get().saveProject();
    if (createdIds.length === 0) set((state) => ({ ...state, lastLibraryError: "Não foi possível montar a referência Promob." }));
  },
  ...
  applyGoldenModuleTest: () => {
    ...
    get().newProject();
    useRoomBuilderStore.getState().applyReferencePreset();
    const id = get().addFurnitureInstance("kitchen-golden-upper-800", { x: 0, y: 1500, z: -1600 }, { width: 800, height: 700, depth: 350 });
    if (id) {
      get().updateFurnitureInstance(id, {
        materialOverrides: {
          body: "mdf-freijo",
    ...
    historyFuture.length = 0;
    get().saveProject();
    if (!id) set((state) => ({ ...state, lastLibraryError: "Não foi possível criar o Golden Module no espaço atual." }));
  },
  ...
  sendMessage: (content) => {
    ...
    const reply = (message: string) => {
      ...
      set((s) => ({ ...s, messages: [...s.messages, { id: `assistant-${Date.now()}`, role: "assistant", content: message, time: replyTime }] }));
    };
    const selectedId = get().selectedId;
    const selected = selectedId ? get().instances.find((instance) => instance.id === selectedId) : undefined;

    const roomMatch = normalized.match(/(?:cozinha|sala|ambiente)[^0-9]{0,24}(\d+(?:[.,]\d+)?)\s*[x×]\s*(\d+(?:[.,]\d+)?)\s*(m|cm|mm)/i);
    if (roomMatch) {
      ...
      if (roomWidth && roomDepth) {
        ...
        reply(`Configurei a sala para ${roomWidth} × ${roomDepth} mm pelo store oficial de ambiente.`);
        return;
      }
    }
    ...
    if (compositionSpec) {
      ...
      if (!layout.valid) {
        reply(`A composição foi interpretada, mas o Layout Engine bloqueou o placement: ${layout.issues.map((issue) => issue.message).join(" ")}`);
        return;
      }
      ...
      for (const moduleSpec of compositionSpec.modules) {
        ...
        const id = get().addFurnitureInstance(moduleSpec.moduleId, placement.positionMm, moduleSpec.dimensionsMm, compositionSpec.thicknessMm, placement);
        if (id) {
          get().updateFurnitureInstance(id, {
            materialOverrides: {
            ...
            hardwareOverrides: compositionSpec.modules.find((item) => item.id === moduleSpec.id)?.kind === "drawer"
              ? { handle: compositionSpec.hardware.handle, slide: compositionSpec.hardware.slide }
              : { handle: compositionSpec.hardware.handle, hinge: compositionSpec.hardware.hinge },
          });
          created.push(id);
        }
      }
      const isLowerOnlyEtapa1 = compositionSpec.modules.length === 4 && compositionSpec.modules.every((module) => module.relation.anchor === "floor") && compositionSpec.modules.every((module) => module.id.startsWith("etapa1-"));
      ...
      if (countertop) {
        ...
        const countertopId = get().addFurnitureInstance(
          ...
          { width: countertop.endX - countertop.startX, height: countertop.thicknessMm, depth: countertop.depthMm },
      ...
      if (created.length === compositionSpec.modules.length + (countertop ? 1 : 0)) {
        reply(`Layout Engine aprovou a sequência em ${compositionSpec.modules.length} ModuleSpec(s), com bancada derivada: parede, âncoras, suporte, bancada contínua e colisões validados. MDF ${compositionSpec.thicknessMm.panelMm} mm aplicado.`);
      } else {
        reply(`O Layout Engine aprovou a geometria, mas ${compositionSpec.modules.length - created.length} módulo(s) não foram construídos pela biblioteca.`);
      }
      return;
    }
    ...
    const isCreationRequest = /(crie|criar|monte|montar|adicione|adicionar|coloque|insira)/i.test(normalized);
    if (requestedModuleId && isCreationRequest && requestedDimensions) {
      const moduleDefinition = ModuleRegistry.get(requestedModuleId);
      const defaults = moduleDefinition?.defaultDimensionsMm ?? { width: 800, height: 870, depth: 580 };
      const [width, height, depth] = requestedDimensions;
      const instanceId = get().addFurnitureInstance(requestedModuleId, undefined, { width: width ?? defaults.width, height: height ?? defaults.height, depth: depth ?? defaults.depth });
      if (instanceId) {
        ...
        if (definition && current) {
          reply(`Criei ${definition.name} com ${current.dimensionsMm.width} × ${current.dimensionsMm.height} × ${current.dimensionsMm.depth} mm, validado pelo placement oficial.`);
          return;
        }
      }
    ...
    if ((requestedModuleId as string | null) === "kitchen-countertop" && normalized.includes("espessura")) {
      const thicknessMatch = normalized.match(/espessura[^0-9]*(\d+(?:[.,]\d+)?)\s*(mm|cm)?/i);
      const thickness = thicknessMatch ? measurementToMm(thicknessMatch[1], thicknessMatch[2]) : null;
      if (selected && thickness) {
        const ok = get().updateFurnitureInstance(selected.id, { dimensionsMm: { ...selected.dimensionsMm, height: thickness } });
        reply(ok ? `A bancada foi reconstruída com espessura de ${thickness} mm.` : "A espessura não passou na validação da bancada.");
        return;
      }
    }

    if (normalized.includes("freij") || normalized.includes("freijo")) {
      if (selected) { get().updateFurnitureInstance(selected.id, { materialOverrides: { ...selected.materialOverrides, body: "mdf-freijo" } }); reply("Apliquei MDF Freijó ao corpo do móvel selecionado usando a mesma operação do Inspector."); }
      else reply("Selecione um móvel para aplicar MDF Freijó.");
      return;
    }
    if (normalized.includes("grafite")) {
      if (selected) { get().updateFurnitureInstance(selected.id, { materialOverrides: { ...selected.materialOverrides, body: "mdf-graphite" } }); reply("Apliquei MDF Grafite ao móvel selecionado."); }
      else reply("Selecione um móvel para aplicar MDF Grafite.");
      return;
    }
    if (normalized.includes("quartzito") || normalized.includes("quartzo") || normalized.includes("porcelanato")) {
      if (selected && selected.moduleDefinitionId === "kitchen-countertop") { get().updateFurnitureInstance(selected.id, { materialOverrides: { ...selected.materialOverrides, countertop: normalized.includes("porcelanato") ? "stone-porcelain" : "stone-quartzite" } }); reply("Atualizei a pedra da bancada selecionada."); }
      else reply("Selecione uma Bancada para trocar a pedra.");
      return;
    }
    if (normalized.includes("led") || normalized.includes("luz quente") || normalized.includes("ilumina")) {
      get().setLightsEnabled(true);
      reply("Ativei a iluminação da cena; o preset atual controla temperatura, intensidade e LED.");
      return;
    }
    if (normalized.includes("golden kitchen") || normalized.includes("cozinha golden") || normalized.includes("montar cozinha profissional")) {
      get().applyGoldenKitchen();
      reply("Montei a Golden Kitchen usando os módulos paramétricos oficiais, Freijó, pedra e o preset de sala profissional.");
      return;
    }
    if (normalized.includes("adicione uma ilha") || normalized.includes("adicionar ilha") || normalized.includes("coloque uma ilha")) {
      const id = get().addFurnitureInstance("kitchen-island-base");
      reply(id ? "Adicionei uma ilha pela operação oficial da Biblioteca." : "Não foi possível adicionar a ilha sem violar o clearance da sala.");
      return;
    }
    if (normalized.includes("gere um render") || normalized.includes("gerar render") || normalized.includes("render realista")) {
    ...
    if (normalized.includes("gere um vídeo") || normalized.includes("gerar vídeo") || normalized.includes("video")) {
      window.dispatchEvent(new CustomEvent("dioris:open-render-final"));
      reply("Abri o painel Render Final; o tour WebM usa a cena atual e a câmera do Planner.");
      return;
    }
    reply("Entendi o pedido. Posso executar materiais, ilha, iluminação, Render Final e vídeo quando o comando corresponder a uma operação disponível.");
  },

  addFurnitureInstance: (moduleId, requestedPositionMm, requestedDimensionsMm, thicknessMm, layout) => {
    ...
    const room = useRoomBuilderStore.getState();
    const y = definition.placementRules.defaultHeightFromFloorMm
      ?? (definition.placementRules.wallMounted ? definition.placementRules.minHeightFromFloorMm : 0);
    ...
    const candidates = requestedPositionMm
      ? [requestedPositionMm]
      : xCandidates.flatMap((candidateX) => zCandidates.map((candidateZ) => ({ x: candidateX, y, z: candidateZ })));
    const roomBounds = { widthMm: room.width, depthMm: room.depth, heightMm: room.height };
  ...
  updateFurnitureInstance: (id, patch) => {
    ...
    set((s) => ({
      ...s,
      lastLibraryError: null,
      instances: s.instances.map((item) => item.id === id
  ...
  duplicateFurnitureInstance: (id) => {
    ...
    const buildAt = (positionMm: { x: number; y: number; z: number }) => buildModule({
      instanceId: newId,
    ...
    if (!outcome.ok) {
      set({ lastLibraryError: outcome.error ?? "Não foi possível duplicar o móvel sem colisão." });
      return;
    }
  ...
  toggleInstanceAnimation: (id: string, partId?: string) => {
    ...
    const target = state.instances.find((item) => item.id === id);
    if (!target) return;
    const currentPart = partId ? target.parts.find((part) => part.id === partId || part.groupId === partId) : undefined;
    const stateKey = currentPart?.groupId ?? currentPart?.id ?? partId;
    const willOpen = currentPart && stateKey ? !(target.openStates?.[stateKey] ?? 0) : !target.isOpen;
    const warnings = willOpen ? validateOpeningClearance(target, state.instances.filter((item) => item.id !== id), stateKey) : [];
    ...
    set((s) => ({
      ...
      instances: s.instances.map((item) => {
        ...
        if (partId) {
          ...
          const key = item.parts.find((part) => part.id === partId || part.groupId === partId)?.groupId ?? partId;
  ...
  openAllAnimations: () => {
    const ids = get().instances.filter((item) => !item.isOpen).map((item) => item.id);
  ...
  setDragPreview: (moduleId, positionMm) => {
    ...
    const roomBounds = { widthMm: room.width, depthMm: room.depth, heightMm: room.height };
    const snapCandidate = get().snapEnabled ? findKitchenSnapCandidate(previewInstance, get().instances, roomBounds) : null;
    ...
    const nearest = get().instances
      .map((instance) => ({ instance, distance: Math.hypot(instance.positionMm.x - effectivePosition.x, instance.positionMm.z - effectivePosition.z) }))
    ...
    const snapInfo = snapCandidate ? `Encaixe ${snapCandidate.target} — ${Math.round(snapCandidate.distanceMm)} mm` : null;
    set({ dragPreview: { moduleId, positionMm: effectivePosition, valid: validation.ok, message: validation.ok ? snapInfo : validation.error ?? "Posição inválida", guides, snapInfo } });
  },
  ...
  setSnapEnabled: (enabled) => set({ snapEnabled: enabled, lastSnapMessage: enabled ? "Snap ativado" : "Snap desativado" }),
  ...
  snapFurnitureInstance: (id) => {
    ...
    set({ lastSnapMessage: `Snap: ${candidate.target === "module" ? "móvel-móvel" : candidate.target}` });
  }
  }))
);
...
if (typeof window !== "undefined") {
  setTimeout(() => {
    ...
    useRoomBuilderStore.subscribe((state, previous) => {
      if (!persistenceReady) return;
      if (state.width !== previous.width || state.depth !== previous.depth || state.height !== previous.height || state.wallThickness !== previous.wallThickness || state.openings !== previous.openings || state.referenceImage !== previous.referenceImage || state.referenceName !== previous.referenceName || state.referenceStyle !== previous.referenceStyle) {
