...
describe("Cozinha Profissional V1", () => {
  ...
  it.each(professionalKitchenModules.map((module) => [module.name, module] as const))(
    "constrói %s por peças independentes",
    (_name, module) => {
      ...
      expect(outcome.parts.every((part) => part.parentInstanceId === `test-${module.id}`)).toBe(
        true,
      );
    },
  );
  ...
  it("classifica volumes e mantém a trajetória de abertura verificável", () => {
    ...
    const flapModule = professionalKitchenModules.find((module) =>
      module.name.toLowerCase().includes("basculante"),
    );
    const flap = flapModule
      ? buildModule({ instanceId: "volume-flap", moduleId: flapModule.id, room })
      : null;
    expect(
      drawer.parts
        .filter((part) => part.interactive?.type === "drawer")
        .every((part) => part.volumeType === "opening"),
    ).toBe(true);
    expect(drawer.parts.some((part) => part.volumeType === "technical")).toBe(true);
    expect(
      flap?.parts.some(
        (part) => part.interactive?.type === "flap" && part.volumeType === "opening",
      ),
    ).toBe(true);
  ...
  it("preserva pivots de dobradiça em portas e basculante", () => {
    const doors = buildModule({
      instanceId: "pivot-doors",
      moduleId: "kitchen-base-2-doors",
      room,
    });
    const flapDefinition = professionalKitchenModules.find((module) =>
      module.name.toLowerCase().includes("basculante"),
    );
    const flap = flapDefinition
      ? buildModule({ instanceId: "pivot-flap", moduleId: flapDefinition.id, room })
      : null;
    expect(
      doors.parts.filter((part) => part.interactive?.type === "door").every((part) => part.pivotMm),
    ).toBe(true);
    expect(
      flap?.parts.find((part) => part.interactive?.type === "flap")?.pivotMm?.y,
    ).toBeGreaterThan(0);
  });

  it("mantém interiores estruturais em torres e cantos", () => {
    const tower = buildModule({
      instanceId: "tower-layout",
      moduleId: "kitchen-tower-oven-microwave",
      room,
    });
    const corner = buildModule({
      instanceId: "corner-pivot",
      moduleId: "kitchen-corner-base",
      room,
    });
    expect(tower.parts.filter((part) => part.role === "divider").length).toBeGreaterThanOrEqual(1);
    expect(
      tower.parts.filter((part) => part.volumeType === "technical").length,
  ...
  it("gera pés reguláveis físicos nos módulos apoiados no piso", () => {
    ...
    const upper = buildModule({
      instanceId: "upper-no-feet",
  ...
  it("insere aéreos e bancadas em alturas estruturais coerentes", () => {
    const upper = professionalKitchenModules.find(
      (module) => module.id === "kitchen-upper-2-doors",
    );
    const countertop = professionalKitchenModules.find(
      (module) => module.id === "kitchen-countertop",
    );
  ...
  it("gera ferragens reais de gola e cava como peças paramétricas", () => {
    const gola = buildModule({
      instanceId: "gola",
      moduleId: "kitchen-base-2-doors",
      hardwareOverrides: { handle: "handle-gola" },
      room,
    });
    const cava = buildModule({
      instanceId: "cava",
      moduleId: "kitchen-drawer-4",
      hardwareOverrides: { handle: "handle-cava" },
      room,
    });
    const golaPart = gola.parts.find(
      (part) => part.role === "hardware" && part.hardwareId === "handle-gola",
    );
    const cavaPart = cava.parts.find(
      (part) => part.role === "hardware" && part.hardwareId === "handle-cava",
    );
  ...
  it("expõe presets profissionais de MDF, pedra e metal", () => {
    expect(
      [
        ...
        "metal-chrome",
        "metal-brass",
      ].every((id) => Boolean(MaterialRegistry.get(id))),
    ).toBe(true);
  });
  ...
  it("encontra Snap entre módulos e para parede dentro da tolerância", () => {
    ...
    const candidate = findKitchenSnapCandidate(second, [first], {
      widthMm: 4800,
      depthMm: 3800,
      heightMm: 2700,
    });
    ...
    const wall = findKitchenSnapCandidate(make("c", 0, -1595), [], {
      widthMm: 4800,
      depthMm: 3800,
      heightMm: 2700,
    });
    ...
    const floor = findKitchenSnapCandidate(floorMoving, [], {
      widthMm: 4800,
      depthMm: 3800,
      heightMm: 2700,
    });
    ...
    const ceiling = findKitchenSnapCandidate(ceilingMoving, [], {
      widthMm: 4800,
      depthMm: 3800,
      heightMm: 2700,
    });
    ...
    const vertical = findKitchenSnapCandidate(verticalMoving, [verticalNeighbor], {
      widthMm: 4800,
      depthMm: 3800,
      heightMm: 2700,
    });
