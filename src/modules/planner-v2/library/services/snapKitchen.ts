...
export function findKitchenSnapCandidate(
  ...
  if (floorDistance > 0 && floorDistance <= toleranceMm)
    candidates.push({
      positionMm: { ...current, y: floorY },
      target: "floor",
      distanceMm: floorDistance,
    });

  const ceilingY = room.heightMm - moving.dimensionsMm.height;
  const ceilingDistance = Math.abs(current.y - ceilingY);
  if (ceilingY >= floorY && ceilingDistance > 0 && ceilingDistance <= toleranceMm)
    candidates.push({
      positionMm: { ...current, y: ceilingY },
      target: "ceiling",
      distanceMm: ceilingDistance,
    });

  const backZ = -room.depthMm / 2 + halfD + KITCHEN_CONFIG.rearGapMm;
  const backDistance = Math.abs(current.z - backZ);
  if (backDistance <= toleranceMm)
    candidates.push({
      positionMm: { ...current, z: backZ },
      target: "back-wall",
      distanceMm: backDistance,
    });

  const leftX = -room.widthMm / 2 + halfW + KITCHEN_CONFIG.sideGapMm;
  const leftDistance = Math.abs(current.x - leftX);
  if (leftDistance <= toleranceMm)
    candidates.push({
      positionMm: { ...current, x: leftX },
      target: "left-wall",
      distanceMm: leftDistance,
    });

  const rightX = room.widthMm / 2 - halfW - KITCHEN_CONFIG.sideGapMm;
  const rightDistance = Math.abs(current.x - rightX);
  if (rightDistance <= toleranceMm)
    candidates.push({
      positionMm: { ...current, x: rightX },
      target: "right-wall",
      distanceMm: rightDistance,
    });
  ...
  for (const other of others) {
    ...
    if (zOverlap) {
      ...
      if (rightGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, x: otherMaxX + halfW },
          target: "module",
          distanceMm: rightGap,
        });
      const leftGap = Math.abs(movingMaxX - otherMinX);
      if (leftGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, x: otherMinX - halfW },
          target: "module",
          distanceMm: leftGap,
        });
    }
    ...
    if (xOverlap) {
      ...
      if (frontGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, z: otherMaxZ + halfD },
          target: "module",
          distanceMm: frontGap,
        });
      const backGap = Math.abs(movingMaxZ - otherMinZ);
      if (backGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, z: otherMinZ - halfD },
          target: "module",
          distanceMm: backGap,
        });
    }
    ...
    if (zAligned && xAligned) {
      ...
      if (topGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, y: other.positionMm.y + other.dimensionsMm.height },
          target: "module",
          distanceMm: topGap,
        });
      const bottomGap = Math.abs(current.y + moving.dimensionsMm.height - other.positionMm.y);
      if (bottomGap <= toleranceMm)
        candidates.push({
          positionMm: { ...current, y: other.positionMm.y - moving.dimensionsMm.height },
          target: "module",
          distanceMm: bottomGap,
        });
    }
  }
