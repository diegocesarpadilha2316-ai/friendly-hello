...
describe("nesting plan — PartDefinition para chapas", () => {
  it("gera nesting MaxRects para a cozinha natural completa", () => {
    ...
    store.newProject();
    store.sendMessage("Crie uma cozinha nessa parede. Quero uma torre de forno e micro-ondas na esquerda, um balcão de 800 com duas portas, um gaveteiro de 600 com quatro gavetas, um balcão de pia de 1200 com duas portas e aéreos em cima. Use MDF 18 mm.");
    const instances = usePlannerStore.getState().instances;
    const parts = instances.flatMap((instance) => instance.parts);
    const plan = buildNestingPlanFromPartDefinitions(parts, { algorithm: "max-rects", kerfMm: 4, marginMm: 10, rotation: "auto", grainMode: "respect", minOffcutMm: 100 });
