...
describe("Kitchen Layout Engine", () => {
  it("posiciona uma cozinha linear sequencialmente e deriva bancada contínua", () => {
    const specs: LayoutModuleSpec[] = [
      {
        ...
        kind: "tower",
        dimensionsMm: { width: 700, height: 2200, depth: 620 },
      ...
      },
      {
        ...
        kind: "drawer",
        dimensionsMm: { width: 600, height: 870, depth: 580 },
      ...
      },
      {
        ...
        kind: "sink",
        dimensionsMm: { width: 1200, height: 870, depth: 580 },
      ...
      },
      {
        ...
        relation: relation("wall", 4, "base"),
      },
    ];
    const result = layoutKitchenModules(specs, createTestWall());
    expect(result.valid).toBe(true);
    expect(result.issues).toEqual([]);
    expect(result.placements.map((placement) => placement.moduleId)).toEqual([
      "tower",
      "base",
      "drawer",
      "sink",
    ...
    expect(
      result.placements.every((placement) => placement.supported && !placement.collision),
    ).toBe(true);
...
it("deriva relações de cooktop, coifa e geladeira sem componentes soltos", () => {
  const specs: LayoutModuleSpec[] = [
    ...
    },
    {
      ...
      relation: relation("wall", 2, "cooktop"),
    },
    {
      id: "fridge",
      moduleId: "kitchen-tower-fridge",
      kind: "tower",
      dimensionsMm: { width: 900, height: 2200, depth: 700 },
      relation: relation("appliance-zone", 3),
    },
  ];
  const result = layoutKitchenModules(specs, createTestWall());
  expect(result.valid).toBe(true);
