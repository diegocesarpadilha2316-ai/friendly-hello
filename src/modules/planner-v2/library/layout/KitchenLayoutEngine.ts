...
function isBase(kind?: string, anchor?: string) {
  return (
    anchor === "floor" &&
    ["base", "drawer", "sink", "cooktop", "corner", "countertop", "peninsula"].includes(kind ?? "")
  );
}
...
export function layoutKitchenModules(
  specs: LayoutModuleSpec[],
  wall: KitchenWall,
): KitchenLayoutResult {
  ...
  for (const spec of ordered) {
    ...
    const isFloorModule =
      isBase(spec.kind, relation.anchor) ||
      relation.anchor === "appliance-zone" ||
      spec.kind === "cooktop";
    ...
    const startX =
      isWallModule && anchorPlacement
        ? anchorPlacement.startX +
          Math.max(0, (anchorPlacement.endX - anchorPlacement.startX - width) / 2)
        : cursorX + clearance;
    ...
    const supported = isFloorModule
      ? bottomY === 0
      : isWallModule
        ? Boolean(relation.anchorModuleId || relation.wallId === wall.id)
        : false;
    let collision = false;
    ...
    if (relation.wallId !== wall.id) {
      issues.push({
        code: "unknown-wall",
        message: `Módulo ${spec.id} aponta para uma parede inexistente.`,
        moduleId: spec.id,
      });
    }
    if (
      endX > wall.originMm.x + wall.widthMm + EPSILON_MM ||
      startX < wall.originMm.x - EPSILON_MM
    ) {
      issues.push({
        code: "module-outside-wall",
        message: `Módulo ${spec.id} ultrapassa os limites da parede.`,
        moduleId: spec.id,
      });
      collision = true;
    }
    for (const opening of wall.openings) {
      ...
      if (
        overlaps(startX, endX, openingStart, openingEnd) &&
        bottomY < opening.topY &&
        topY > opening.bottomY
      ) {
        issues.push({
          code: "module-wall-opening",
          message: `Módulo ${spec.id} atravessa a abertura ${opening.id}.`,
          moduleId: spec.id,
        });
        collision = true;
      }
    ...
    for (const previous of placements) {
      ...
      const verticalOverlap =
        bottomY < previous.topY - EPSILON_MM && topY > previous.bottomY + EPSILON_MM;
      const depthOverlap =
        Math.abs(positionMm.z - previous.positionMm.z) <
        (depthMm + previous.depthMm) / 2 - EPSILON_MM;
      if (
        verticalOverlap &&
        depthOverlap &&
        overlaps(startX, endX, previous.startX, previous.endX)
      ) {
        issues.push({
          code: "module-collision",
          message: `Módulo ${spec.id} colide com ${previous.moduleId}.`,
          moduleId: spec.id,
        });
        collision = true;
      }
    ...
    if (!supported) {
      issues.push({
        ...
        message: `Módulo ${spec.id} não está apoiado ou ancorado.`,
        moduleId: spec.id,
      });
    }
    ...
    if (isFloorModule || relation.anchor === "appliance-zone") {
      ...
      if (!anchor)
        issues.push({
          code: "upper-without-anchor",
          message: `Aéreo ${spec.id} não possui módulo inferior de referência.`,
          moduleId: spec.id,
        });
    }
  ...
  const floorPlacements = placements.filter(
    (placement) =>
      placement.anchor === "floor" &&
      !["tower", "fridge", "appliance"].includes(
        specs.find((spec) => spec.id === placement.moduleId)?.kind ?? "",
      ),
  );
  if (floorPlacements.length > 0) {
    ...
    const supported = floorPlacements.every(
      (placement) => placement.supported && !placement.collision,
    );
    countertops.push({
      id: "countertop-wall-test",
      wallId: wall.id,
      supportModuleIds,
      startX,
      endX,
      ...
      thicknessMm: COUNTERTOP_THICKNESS_MM,
      supported,
    });
    if (!supported)
      issues.push({
        code: "countertop-unsupported",
        message: "A bancada contínua não possui suporte integral dos módulos inferiores.",
      });
    ...
    for (const spec of specs) {
      ...
      if (spec.kind === "sink" || spec.kind === "cooktop") {
        ...
        technicalRelationships.push({
          id: `${spec.kind}-${spec.id}`,
          type: spec.kind,
          parentModuleId: spec.id,
          countertopId: countertop.id,
          centerX: (placement.startX + placement.endX) / 2,
          topY: countertop.topY,
          clearanceMm: 40,
          valid: parentSupported && countertop.supported,
        });
        if (!parentSupported)
          issues.push({
            code: `${spec.kind}-without-countertop`,
            message: `${spec.kind} ${spec.id} não está apoiado em um balcão compatível.`,
            moduleId: spec.id,
          });
      }
      if (spec.kind === "hood") {
        const target = spec.relation.anchorModuleId
          ? placements.find((item) => item.moduleId === spec.relation.anchorModuleId)
          : placements.find(
              (item) =>
                specs.find((candidate) => candidate.id === item.moduleId)?.kind === "cooktop",
            );
        ...
        technicalRelationships.push({
          ...
          type: "hood",
          parentModuleId: spec.id,
          targetCooktopId: target?.moduleId,
          centerX: target
            ? (target.startX + target.endX) / 2
            : (placement.startX + placement.endX) / 2,
          topY: placement.bottomY,
          clearanceMm: 650,
          valid: validHood,
        });
        if (!validHood)
          issues.push({
            code: "hood-without-cooktop",
            message: `Coifa ${spec.id} não possui cooktop-alvo.`,
            moduleId: spec.id,
          });
      }
    }
 ...
 */
export function layoutKitchenAcrossWalls(
  specs: LayoutModuleSpec[],
  walls: KitchenWall[],
): KitchenLayoutResult {
  ...
  for (const [wallId, wallSpecs] of grouped) {
    ...
    if (!wall) {
      results.push({
        ...
        applianceZones: [],
        technicalRelationships: [],
        issues: [{ code: "unknown-wall", message: `Parede ${wallId} não existe.` }],
        valid: false,
      });
      continue;
    }
