...
import {
  computeHealth, mapBackup, mapHistory, mapIntegrity, mapPlan, mapRestore,
  mapSchedule, mapSnapshot, mapTarget, recordRestore,
} from "./manager.server";
...
export const recoverySnapshot = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<RecoverySnapshot> => {
    const s = context.supabase; const t = context.tenantId;
    const [targets, schedules, backups, snapshots, restores, integrity, plans, history] = await Promise.all([
      s.from("recovery_targets").select("*").eq("company_id", t).order("kind").limit(100),
      s.from("recovery_schedules").select("*").eq("company_id", t).order("next_run_at").limit(100),
      s.from("recovery_backups").select("*").eq("company_id", t).order("created_at", { ascending: false }).limit(100),
      s.from("recovery_snapshots").select("*").eq("company_id", t).order("created_at", { ascending: false }).limit(100),
      s.from("recovery_restores").select("*").eq("company_id", t).order("created_at", { ascending: false }).limit(100),
      s.from("recovery_integrity").select("*").eq("company_id", t).order("checked_at", { ascending: false }).limit(100),
      s.from("recovery_dr_plans").select("*").eq("company_id", t).order("updated_at", { ascending: false }).limit(50),
      s.from("recovery_history").select("*").eq("company_id", t).order("bucket_at", { ascending: false }).limit(120),
    ]);
    ...
    return {
      targets: ts, schedules: sc, backups: bs,
...
const targetSchema = z.object({
  ...
  slug, name: z.string().min(1).max(160),
  kind: z.enum(["database","storage","assets","configuration","sdk","plugins","cache","logs","events","notifications"]),
  destination: z.enum(["internal","s3","r2","gcs","azure_blob","b2","supabase_pitr"]).default("internal"),
...
export const recoveryTargetUpsert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const payload = {
      ...
      destination: data.destination, retention_days: data.retentionDays,
      encryption: data.encryption, enabled: data.enabled,
      metadata: data.metadata, updated_at: new Date().toISOString(),
    };
    const q = data.id
      ? context.supabase.from("recovery_targets").update(payload).eq("id", data.id).eq("company_id", context.tenantId).select("*").single()
      : context.supabase.from("recovery_targets").upsert(payload, { onConflict: "company_id,slug" }).select("*").single();
...
export const recoveryTargetDelete = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase.from("recovery_targets")
      .delete().eq("company_id", context.tenantId).eq("id", data.id);
    if (error) throw new Error(error.message);
...
export const recoveryScheduleUpsert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const payload = {
      company_id: context.tenantId, target_id: data.targetId,
      cron: data.cron, strategy: data.strategy, enabled: data.enabled,
      next_run_at: data.nextRunAt ?? null, updated_at: new Date().toISOString(),
    };
    const q = data.id
      ? context.supabase.from("recovery_schedules").update(payload).eq("id", data.id).eq("company_id", context.tenantId).select("*").single()
...
export const recoveryScheduleDelete = createServerFn({ method: "POST" })
  ...
  .inputValidator((raw: unknown) => idOnly.parse(raw))
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase.from("recovery_schedules")
      .delete().eq("company_id", context.tenantId).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true as const };
  });
...
const backupSchema = z.object({
  ...
  status: z.enum(["queued","running","completed","failed","verified","expired","cancelled"]).default("queued"),
...
export const recoveryBackupRecord = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const finished = data.status === "completed" || data.status === "verified"
      || data.status === "failed" || data.status === "cancelled";
    const { data: row, error } = await context.supabase.from("recovery_backups").insert({
      company_id: context.tenantId,
      target_id: data.targetId ?? null,
      target_slug: data.targetSlug, kind: data.kind,
      strategy: data.strategy, trigger: data.trigger, status: data.status,
...
export const recoverySnapshotRecord = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: row, error } = await context.supabase.from("recovery_snapshots").insert({
      company_id: context.tenantId,
...
export const recoveryIntegrityRecord = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: row, error } = await context.supabase.from("recovery_integrity").insert({
      company_id: context.tenantId,
      backup_id: data.backupId,
      check_kind: data.checkKind, status: data.status,
      detail: data.detail ?? null, duration_ms: data.durationMs ?? null,
    }).select("*").single();
    if (error) throw new Error(error.message);
    if (data.status === "pass") {
      await context.supabase.from("recovery_backups")
        .update({ status: "verified" })
        .eq("company_id", context.tenantId).eq("id", data.backupId);
    }
...
const planSchema = z.object({
  id: z.string().uuid().optional(),
  slug, name: z.string().min(1).max(160),
  rtoMinutes: z.number().int().min(1).max(1440 * 30).default(60),
  rpoMinutes: z.number().int().min(1).max(1440 * 30).default(15),
...
export const recoveryPlanUpsert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const payload = {
      ...
      rto_minutes: data.rtoMinutes, rpo_minutes: data.rpoMinutes,
      replication: data.replication, failover: data.failover,
      status: data.status, metadata: data.metadata,
      updated_at: new Date().toISOString(),
    };
    const q = data.id
      ? context.supabase.from("recovery_dr_plans").update(payload).eq("id", data.id).eq("company_id", context.tenantId).select("*").single()
      : context.supabase.from("recovery_dr_plans").upsert(payload, { onConflict: "company_id,slug" }).select("*").single();
    const { data: row, error } = await q;
    if (error) throw new Error(error.message);
...
export const recoveryPlanDelete = createServerFn({ method: "POST" })
  ...
  .inputValidator((raw: unknown) => idOnly.parse(raw))
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase.from("recovery_dr_plans")
      .delete().eq("company_id", context.tenantId).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true as const };
  });
...
export const recoveryPlanDrill = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: row, error } = await context.supabase.from("recovery_dr_plans")
      .update({
      ...
      .eq("company_id", context.tenantId).eq("id", data.id).select("*").single();
    if (error) throw new Error(error.message);
    return mapPlan(row as Record<string, unknown>);
  });
