...
export const listPublicPlans = createServerFn({ method: "GET" }).handler(async (): Promise<PublicPlanDTO[]> => {
  ...
  if (error) return [];
  return (data ?? []).map((r: {
    key: string; label: string; monthly_credits: number; price_cents: number;
    sort_order: number; features: unknown;
  }) => ({
    key: r.key,
    label: r.label,
...
export const listPublicCreditPacks = createServerFn({ method: "GET" }).handler(async (): Promise<PublicCreditPackDTO[]> => {
  const sb = publicClient();
  const { data, error } = await sb
    .from("credit_packs")
  ...
  if (error) return [];
  return (data ?? []).map((r: {
    key: string; label: string; credits: number; price_cents: number;
    currency: string; bonus_pct: number; sort_order: number;
  }) => ({
    key: r.key,
    label: r.label,
    credits: r.credits,
